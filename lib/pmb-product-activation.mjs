...
export function getAssignedPmbActivationTargets(rows) {
  return [...buildVerifiedKegSlotMap(rows).values()].map((row) => ({
    name: clean(row.product),
    plu: validPlu(row.plu),
    source: `Assigned tap ${row.tapNumber || `${row.deviceId}:${row.lineNum}`}`,
  }));
}
...
function createClient(env) {
  ...
  return {
    ...
    taps: () => getTapConfigRows(config, { timeoutMs: 5000 }),
...
export async function activateQueuedPmbProducts({
  ...
  const summary = { checked: 0, alreadyActive: 0, activated: [], issues: [], verified: false };
  let pending = null;
  try {
    ...
    let assignedTargets = [];
    try {
      assignedTargets = getAssignedPmbActivationTargets(await pmb.taps());
      targets.push(...assignedTargets);
    } catch (error) {
      summary.issues.push({ source: "Assigned taps", code: error.code || "PMB_ASSIGNED_PRODUCTS_UNAVAILABLE" });
    }
    try {
      const dashboard = await readDashboard();
      const kegs = await readKegs();
      targets.push(...getQueuedPmbActivationTargets(dashboard, kegs));
    } catch (error) {
      // A queued-list outage must not prevent repair of verified current assignments.
      summary.issues.push({ source: "On Deck/Coming Soon", code: error.code || "PMB_ACTIVATION_STATE_UNAVAILABLE" });
    }
    if (targets.length) {
      // Resolve every saved identity first. Never create a new product or guess a similar name.
      const catalog = await pmb.list();
      const resolved = new Map();
      for (const target of targets) {
        try {
          for (const product of resolvePmbActivationTargets([target], catalog)) resolved.set(product.plu, product);
        } catch (error) {
          summary.issues.push({ name: target.name, plu: target.plu, code: error.code || "PMB_ACTIVATION_IDENTITY_UNVERIFIED" });
        }
      }
      ...
      for (const product of products) {
        try {
        await beforeWrite();
        const assigned = assignedTargets.some((target) => target.plu === product.plu);
        const catalogProduct = catalog.find((entry) => validPlu(entry.plu) === product.plu);
        // Avoid opening every active tap's edit form during the short preparation window.
        if (assigned && [1, "1", true].includes(catalogProduct?.is_active)) {
        ...
        const guardActivation = async () => {
          if (assigned) {
            const liveTargets = getAssignedPmbActivationTargets(await pmb.taps());
            if (!liveTargets.some((target) => target.plu === product.plu
              && identityName(target.name) === identityName(product.name))) {
              throw activationError("PMB_ACTIVATION_ASSIGNMENT_CHANGED", "Product is no longer verified on a tap; activation was skipped.");
            }
          }
          await beforeWrite();
        };
        pending = product;
        await pmb.save(entries, form.cookieJar, guardActivation);
        ...
        await recordActivity({
          area: "PMB", action: "reactivated PMB product", role: "system",
          summary: `${product.name} (PLU ${product.plu}) verified active; other product fields preserved. Source: ${assigned ? "current tap assignment" : "On Deck/Coming Soon"}.`,
        }).catch(() => {});
        } catch (error) {
          summary.issues.push({ ...product, code: error.code || "PMB_ACTIVATION_FAILED", saveUnconfirmed: Boolean(pending) });
          pending = null;
          if (error.code === "PMB_REPAIR_WINDOW_CLOSED") throw error;
          // Never retry an uncertain product save within this preparation run.
        }
      }
    }
    await beforeWrite();
    summary.verified = summary.issues.length === 0;
    await recordActivity({
      area: "PMB", action: "checked queued product activation", role: "system",
      summary: `${summary.checked} assigned/On Deck/Coming Soon products verified active; ${summary.activated.length} activated; ${summary.issues.length} product issues retained for review.`,
    }).catch(() => {});
    return summary;
  } catch (error) {
    ...
    await recordActivity({
      ...
      summary: `${error.code || "PMB_ACTIVATION_FAILED"}: Product preparation needs attention.${pending ? ` Unverified save: ${pending.name}.` : ""} Verified activations: ${summary.activated.length}. Scheduled configuration repair remains independent.`,
    }).catch(() => {});
    throw error;
  }
}
