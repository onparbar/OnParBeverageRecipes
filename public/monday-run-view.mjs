...
export function renderMondayRunCompact(run) {
  ...
  return `
    <section class="monday-run monday-run--compact${run.complete ? " is-complete" : ""}" aria-label="Weekly Plan">
      <header class="monday-run__header"><div><h2>Weekly Plan</h2><span>${run.complete ? "Complete" : `Step ${run.nextIndex + 1} of ${run.steps.length}`}</span></div><div class="monday-run__header-actions">${weekBadge(run)}${actionButton(run)}</div></header>
