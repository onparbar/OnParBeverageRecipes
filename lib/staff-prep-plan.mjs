...
export function buildStaffPrepPlan(recommendations = {}) {
  ...
  const items = plan.prep.cocktails.map((item) => {
    const id = item.prepAdditionId ? `cocktail-addition:${item.prepAdditionId}` : itemId(item.name);
    const legacyId = item.prepAdditionId ? "" : legacyItemId(item.name);
    ...
    return {
      ...
      ...(item.prepAdditionId ? { prepAdditionId: item.prepAdditionId } : {}),
  ...
  const liquorRefills = plan.orders.liquorTapBottles.map((item) => {
    ...
    return {
      ...
      actualQuantity: completed
        ? Math.max(1, Number(saved.actualQuantity) || Number(item.quantity) || 1)
...
export function applyStaffPrepPlanUpdate(
  ...
  const actualQuantity = target.kind === "liquor-refill"
    ? Number(payload.actualQuantity ?? target.quantity)
    : null;
  if (payload.completed && target.kind === "liquor-refill"
    && (!Number.isInteger(actualQuantity) || actualQuantity < 1 || actualQuantity > 99)) {
    throw new StaffPrepPlanError(
      "STAFF_LIQUOR_QUANTITY_REQUIRED",
      "Enter the number of bottles actually added before checking it off.",
    );
  }
  ...
  const prepChecklist = isPlainRecord(recommendations.prepChecklist)
    ? { ...recommendations.prepChecklist }
    : {};
  const legacyTargetId = target.kind === "liquor-refill" || target.prepAdditionId ? "" : legacyItemId(target.name);
  if (legacyTargetId && legacyTargetId !== target.id && isPlainRecord(prepChecklist[legacyTargetId])) {
    ...
    currentPlan.items
      .filter((item) => !item.prepAdditionId && legacyItemId(item.name) === legacyTargetId)
      .forEach((item) => {
  ...
  if (payload.completed) {
    ...
    prepChecklist[target.id] = {
      completed: true,
      preparedBy,
      ...(target.kind === "liquor-refill" ? { actualQuantity } : {}),
