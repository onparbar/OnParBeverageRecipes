...
import { getCocktailRecipeSourceTitle } from "../public/cocktail-recipe-yields.mjs";
...
function identity(value) {
  return clean(value).normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[’']/g, "").toLowerCase()
    ...
    .replace(/\bcold brew (?:coffee|concentrate)\b/g, "cold brew")
...
function canonicalInventoryName(value) {
  const normalized = clean(value).replace(/\b1\.75(?:\s*(?:ml|l))?\b/gi, " ").replace(/\s+/g, " ").trim();
  if (/^tito'?s(?:\s+(?:handmade\s+)?vodka)?$/i.test(normalized)) return "Tito's";
  if (/^jose cuervo(?:\s+silver)?(?:\s+tequila)?$/i.test(normalized)) return "Jose Cuervo Silver";
  return normalized;
}
...
function baseInventoryRows(rows) {
  ...
  rows.forEach((row) => {
    ...
    const name = canonicalInventoryName(first);
    items.push({ id: slug(name), name, baseline, group: section, packSize });
  });
  return items;
}
...
async function buildInventoryCatalog(state) {
  ...
  const onHandOverrides = state?.current?.onHandOverrides && typeof state.current.onHandOverrides === "object"
  ...
  [...base, ...customItems, ...snapshotItems].forEach((item) => {
    ...
    byId.set(id, {
      ...
      baseline: Math.max(0, number(
        Object.prototype.hasOwnProperty.call(onHandOverrides, id)
          ? onHandOverrides[id]
          : item?.onHandDisplay ?? item?.onHand ?? item?.baseline ?? existing.baseline,
      )),
...
export function findCatalogItem(catalog, { id = "", name = "", raw = "" } = {}) {
  ...
  const preferredIds = {
    titos: ["tito-s"],
    "jose cuervo": ["jose-cuervo-silver"],
    "jose cuervo silver": ["jose-cuervo-silver"],
  }[key];
  if (preferredIds) {
    for (const preferredId of preferredIds) {
      const preferred = catalog.filter((item) => item.id === preferredId);
  ...
  const legacyCanonical = key === "titos"
    ? exact.filter((item) => item.id === "titos")
    : [];
  if (legacyCanonical.length === 1) return legacyCanonical[0];
...
function usesWholeBottles(ingredient) {
  const packageUnit = clean(ingredient?.packageUnit).toLowerCase();
  if (packageUnit.startsWith("bottle")) return true;
  return /(\d+(?:\.\d+)?)\s*(?:\([^)]*\)\s*)?(?:bottles?|btls?)\b/i.test(clean(ingredient?.raw));
}

function isUncountedPrepIngredient(ingredient) {
  const name = clean(ingredient?.name)
    .replace(/^\d+(?:\.\d+)?\s+/, "")
    .replace(/^gallons?\s+/, "");
  return /^(?:(?:strawberry )?lemonade|cranberry(?: juice)?|sweet tea|simple syrup|mint|blue dot juice)$/i.test(name);
}
...
export function buildRecipeInventoryContributions(recipe, catalog, target = {}) {
  ...
  const batchSizeOz = number(target?.batchSizeOz);
  const nominalScale = recipeYieldOz > 0 && batchSizeOz > 0 ? batchSizeOz / recipeYieldOz : 1;
  // The prep-plan yield can round a recipe's ingredient sum by an ounce.
  // Treat that as the same batch, not an instruction to use fractional bottles.
  const scale = batchSizeOz > 0 && Math.abs(batchSizeOz - recipeYieldOz) <= 1.001
    && Math.abs(nominalScale - 1) <= 0.001 ? 1 : nominalScale;
  ...
  inventoryIngredients.forEach((ingredient) => {
    ...
    // These food-department mixers and garnishes are not counted beverage
    // stock. Keep their ounces in the batch yield above so counted bottles
    // retain the correct scale.
    if (isUncountedPrepIngredient(ingredient)) return;
    if (getUncountedInventoryAmount(ingredient) !== null) return;
    const item = findCatalogItem(catalog, {
      id: ingredient.inventoryItemId || ingredient.inventoryId || ingredient.itemId,
      name: ingredient.name,
      raw: ingredient.raw,
    });
    if (!item) throw new InventoryContributionError(
      "INVENTORY_IDENTITY_REVIEW_REQUIRED",
      `Match counted ingredient "${clean(ingredient.name)}" to inventory before completing ${clean(recipe.title)}.`,
      409, { ingredient: clean(ingredient.name), recipe: clean(recipe.title) },
    );
    if (item.notCounted === true || getUncountedInventoryAmount(item) !== null) return;
    const calculatedUnits = packageUnits(ingredient, item) * scale * batches;
    // Bottle-based recipes use complete bottles in service. Ounce totals in
    // the source sheets are display values and may be rounded, so they must
    // never turn a full-bottle recipe into a fractional inventory movement.
    const units = usesWholeBottles(ingredient)
      ? Math.ceil(calculatedUnits - 1e-9)
      : calculatedUnits;
    if (!(units > 0)) throw new InventoryContributionError(
      "INVENTORY_PACKAGE_REVIEW_REQUIRED",
      `Set the package size or quantity for "${clean(ingredient.name)}" before completing ${clean(recipe.title)}.`,
      409, { ingredient: clean(ingredient.name), recipe: clean(recipe.title) },
    );
...
function findRecipe(recipes, target) {
  ...
  if (exact.length) return exact.length === 1 ? specializeRecipeForTarget(exact[0], target) : null;
  // Use the same named recipe aliases as the tap batch-size model. For
  // example, Pink Lemonade uses the existing Strawberry Lemonade recipe.
  // Exact saved recipes above take precedence over these legacy aliases.
  const sourceKeys = new Set([target?.name, target?.displayName]
    .map(getCocktailRecipeSourceTitle).map(titleIdentity).filter(Boolean));
  const aliased = recipes.filter((recipe) => (
    sourceKeys.has(titleIdentity(recipe.title)) || sourceKeys.has(titleIdentity(recipe.sourceTitle))
  ));
  if (aliased.length) return aliased.length === 1 ? specializeRecipeForTarget(aliased[0], target) : null;
...
async function applySources(sources, role) {
  ...
  const shortfalls = state.current.contributionShortfalls || {};
  const inventoryReviewItemIds = Object.keys(shortfalls);
  return {
    appliedItemCount: sources.reduce((total, source) => total + source.contributions.length, 0),
    revision: state.revision,
    ...(inventoryReviewItemIds.length ? {
      shortfalls,
      warning: `Checklist saved. Recorded inventory movements exceed stock for: ${inventoryReviewItemIds.map(id => id.replace(/-/g, " ")).join(", ")}. Recount these items to reconcile the shortage.`,
    } : {}),
  };
}
...
export async function planReceiptInventoryContributions(tracking, vendorId) {
  ...
  for (const line of vendor.items || []) {
    ...
    // Beer receipts update cooler keg counts atomically with the receipt, not
    // cabinet inventory, even if a catalog product happens to share its name.
    ...
    if (getUncountedInventoryAmount(item) !== null) continue;
    const current = byItem.get(item.id) || { id: item.id, quantity: 0, baseline: item.baseline };
...
export { loadRecipes as readPrepRecipes, findRecipe as findPrepRecipe };
