...
export function mountTapPerformance(root, getCosts) {
  ...
  root.innerHTML = `<header class="tap-performance-heading"><div><p class="eyebrow">POUR MY BEER / DAILY HISTORY</p><h2>Tap Performance</h2></div><span class="table-note">Eastern time</span></header>
    <form class="tap-performance-filters">
      ...
      <button class="primary-button" type="submit">Load saved</button>
    ...
    <details class="tap-performance-notes"><summary>How these figures are calculated</summary>
      <p>PMB recorded charges are not POS-settled revenue. Taxes, refunds handled outside PMB, labor, fees and overhead are not included. Gross profit is an estimate using the cost snapshot saved with each daily import, not a claim of historical accounting profit.</p>
      <p>Historical tap matches based on today’s product assignment are labeled inferred. Empty, failed and disagreeing reports are never treated as confirmed zero days. Matching overlapping reads reduce boundary omissions but are not a vendor-certified completeness guarantee.</p>
    </details>`;
  ...
  function render() {
    ...
    for (const row of filtered) {
      ...
      const key = `${date}|${row.tapNumber}|${row.suggestedTapNumber}|${row.plu}|${row.assignmentEvidence}`;
      const group = groups.get(key) || { ...row, day: date, members: [] };
    ...
    root.querySelector('[data-results]').innerHTML = `<p class="table-note">${reports.length} saved days · ${missingDays.length} missing days · ${partial} unverified or partial days. Totals below cover captured records only.</p>
      ...
      <div class="inventory-table-wrap"><table class="inventory-table"><thead><tr>${input('group').value === 'day' ? '<th>Day</th>' : ''}<th>Tap / Product</th><th>Ounces</th><th>PMB sales</th><th>Est. cost</th><th>Est. gross profit</th></tr></thead><tbody>
      ${[...groups.values()].map(g => `<tr>${input('group').value === 'day' ? `<td>${escape(g.day)}</td>` : ''}<td><strong>${g.tapNumber || g.suggestedTapNumber ? `Tap ${g.tapNumber || g.suggestedTapNumber}${g.suggestedTapNumber ? ' (inferred)' : ''}` : 'Unassigned'}</strong><br>${escape(g.product)}<small class="table-note">${escape(g.assignmentEvidence)}</small></td><td>${amount(g.members.reduce((s, r) => s + r.volumeOz, 0))}</td><td>${dollars(sumKnown(g.members, 'revenue'))}</td><td>${dollars(sumKnown(g.members, 'estimatedCost'))}</td><td>${dollars(sumKnown(g.members, 'estimatedGrossProfit'))}</td></tr>`).join('') || `<tr><td colspan="6">No captured pours for this selection. Missing data is not zero usage.</td></tr>`}
      </tbody></table></div>`;
  }
