...
export function buildWeeklyOrderTracking(recommendations = {}, now = new Date()) {
  ...
  return {
    ...
    notReceivedCount: items.filter((item) => ["partial", "not-received", "rejected"].includes(item.status)).length,
    notReceivedItems: items.filter((item) => ["partial", "not-received", "rejected"].includes(item.status)),
  };
}
...
export function applyWeeklyOrderTrackingUpdate(
  ...
  return {
    ...recommendations,
    weeklyOrderTracking: { vendors, receipts, deliveryNotes, orderDrafts, orderAdjustments, orderPolicy },
  };
}
