...
export function parseReportHistoryRequest(params) {
  ...
  if (!['catalog', 'menu', 'home', 'reporting', 'history', 'poured', 'products'].includes(view)) throw Object.assign(new Error('Choose catalog, menu, home, or products.'), { status: 422 });
...
export async function readPmbReportHistory(input, { config = getConfig(), readPage = resilientRead, readProduct = readProductForm } = {}) {
  ...
  const path = input.view === 'catalog' ? '/pages/products' : input.view === 'home' ? '/' : ['reporting','history'].includes(input.view) ? '/pages/reporting' : '/pages/tapconfig';
  ...
  if (input.view === 'history') {
    const tables = [...result.raw.matchAll(/<table\b([^>]*)>([\s\S]*?)<\/table>/gi)].filter(m => ['dtb_ppbvm','dtb_ppbvw','dtb_ppbvd','dtb_ppbmm'].includes(attribute(m[1],'id'))).map(m => ({id:attribute(m[1],'id'),...describePmbProductReportPage(`<table>${m[2]}</table>`).tables[0]}));
    if (!tables.some(t => t.id === 'dtb_ppbvm')) throw Object.assign(new Error('Monthly poured-volume table unavailable.'), {status:502});
    return {tables,coverage:'Use the dates in each table header; PMB export date filters do not apply to these tables.'};
  }
  const described = describePmbProductReportPage(result.raw);
  if (input.view === 'reporting') {
    delete described.tables;
    described.tableHeaders = [...result.raw.matchAll(/<table\b([^>]*)>([\s\S]*?)<\/table>/gi)].map(m => ({id:attribute(m[1],'id'),headers:[...m[2].matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/gi)].map(x=>plain(x[1]))}));
    described.reportScripts = [...result.raw.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi)].flatMap(m => {
      const src=attribute(m[1],'src');
      const lines=m[2].split('\n').filter(line => /tppb_v|ajax|sAjaxSource|iDisplayLength|pageLength|serverSide|reporting\//i.test(line) && line.length<1200 && !/password|token|cookie|secret|authorization/i.test(line));
      return [src,...lines].filter(Boolean);
    });
...
export async function readPouredReport(input, config, readPage) {
  ...
  return {startDate:input.start,endDate:input.end,unit:input.unit,dateFilterAccepted:true,exportRangeVerified:false,report:'Poured volume by product',csv:result.raw};
}
