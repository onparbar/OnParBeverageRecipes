...
function normalizeData(value, { requireEveryField = false } = {}) {
  ...
  const allowed = new Set([...OBJECT_FIELDS, "recommendations", "inputEditState"]);
...
export function createSharedKegParAgentStore({ env = process.env, fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
  ...
  return Object.freeze({ read, editInputs, initialize: (payload, role) => save({ ...payload, initialize: true }, role), replace: (payload, role) => save(payload, role) });
}
