...
const HIGH_COVERAGE_MAIN_BEER_AVG = 0.7;
const HIGH_COVERAGE_MAIN_BEER_BACKUPS = 2;
const HIGH_COVERAGE_MAIN_BEER_MAX_ORDER = 3;
const HIGH_COVERAGE_MAIN_BEER_NAMES = Object.freeze([
  "michelob ultra",
  "miller lite",
  "garage beer lime",
  "angry orchard",
...
const LIQUOR_TAP_ORDER_BOTTLES = 2;
...
export function buildRawRecommendation(tap, level, weeklyEntries, state, settings, sharedWeeklyUsageItem = null) {
  ...
  if (isLiquor) {
    ...
    const suggestedBottleOrderQty = currentStockOunces < targetStockOunces
      ? LIQUOR_TAP_ORDER_BOTTLES
      : 0;
    const weeksOfStock = avgWeeklyOunces > 0 ? round(currentStockOunces / avgWeeklyOunces, 2) : 99;
    const reason = suggestedBottleOrderQty
      ? `Current keg ${currentStockOunces} oz is below ${targetStockOunces} oz: ${avgWeeklyOunces} oz/week plus 100 oz. Order 2 bottles for this physical liquor tap.`
  ...
  const normalizedBeerName = clean(tap.name || tap.brand)
    .toLowerCase()
    .replace(/\s+[123]\s*$/, "");
  const highCoverageMainBeer = isMainBeer && (
    avgWeeklyKegs >= HIGH_COVERAGE_MAIN_BEER_AVG
    || HIGH_COVERAGE_MAIN_BEER_NAMES.some((name) => normalizedBeerName.includes(name))
  );
  const beerReserveKegs = isMainBeer
    ? MAIN_BEER_RESERVE_KEGS
    : KARAOKE_BEER_RESERVE_KEGS;
  const usageTargetStockKegs = avgWeeklyKegs + beerReserveKegs;
  const highCoverageTargetStockKegs = highCoverageMainBeer
    ? liveFraction + HIGH_COVERAGE_MAIN_BEER_BACKUPS
    : 0;
  const targetStockKegs = round(Math.max(usageTargetStockKegs, highCoverageTargetStockKegs), 2);
  const gapKegs = round(targetStockKegs - currentStockKegs, 3);
  const configuredMaxOrderPerTap = Math.max(1, Math.round(toNumber(settings?.maxOrderPerTap) || DEFAULT_SETTINGS.maxOrderPerTap));
  const maxOrderPerTap = highCoverageMainBeer
    ? Math.max(configuredMaxOrderPerTap, HIGH_COVERAGE_MAIN_BEER_MAX_ORDER)
    : configuredMaxOrderPerTap;
  const calculatedOrderQty = gapKegs > 0 ? Math.ceil(gapKegs) : 0;
  ...
  if (highCoverageMainBeer) {
    reasonParts.push(`High-coverage Main tap: keep ${HIGH_COVERAGE_MAIN_BEER_BACKUPS} unopened backup kegs.`);
  }
  if (orderCapApplied) {
