...
export function createSharedKegParAgentStore({ env = process.env, fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
  async function request(method, filters = {}, body) {
    ...
    let response;
    try { response = await fetchImpl(url, { method, headers: headers(config.secret, method), cache: "no-store", ...(body === undefined ? {} : { body: JSON.stringify(body) }) }); }
    catch { throw fail("KEG_STATE_UNAVAILABLE", "Shared Keg Levels storage is unavailable.", 503); }
    let result = null;
