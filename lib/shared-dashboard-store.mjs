...
const FIELD_SHAPES = Object.freeze({
  ...
  monitoring: Object.freeze({
    ohioComplianceAcknowledgement: "object",
    comingSoonFollowUps: "object",
  }),
});
...
export function projectSharedDashboardStateForRole(state, role) {
  ...
  return {
    ...
    data: {
      ...
      monitoring: emptyData.monitoring,
    },
  };
}
...
function normalizeCompleteData(
  value,
  {
    ...
    allowLegacyMissingPmbQueue = false,
    allowLegacyMissingMonitoring = false,
  } = {},
) {
  ...
  for (const [group, fields] of Object.entries(FIELD_SHAPES)) {
    ...
    const isLegacyMonitoringGroup = allowLegacyMissingMonitoring
      && group === "monitoring"
      && groupValue === undefined;
    if (requireEveryField && !isLegacyMonitoringGroup && !isPlainObject(groupValue)) {
    ...
    for (const [field, expectedShape] of Object.entries(fields)) {
      ...
      const isLegacyMonitoringField = allowLegacyMissingMonitoring && group === "monitoring";
      if (requireEveryField && !isLegacyQueueField && !isLegacyMonitoringField && !Object.hasOwn(groupValue, field)) {
        throw storeError(
          "INVALID_SHARED_STATE",
...
function normalizeRow(row) {
  ...
  try {
    data = normalizeCompleteData(row.data || {}, {
      ...
      allowLegacyMissingMonitoring: true,
    });
  } catch (error) {
