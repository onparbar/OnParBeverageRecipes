...
import { MINIMUM_KEG_CUSHION, getEightWeekPeakUsage } from "../public/keg-demand-policy.mjs";
...
const LIQUOR_BOTTLE_SIZE_OVERRIDES = [
  ...
  { names: ["buffalo trace"], ounces: 33.814 },
...
export async function getKegWallItems() {
...
export async function writeParAgentState(nextState, { expectedRevision, initialize = false, role = "owner", inventoryPlan = null } = {}) {
  const payload = {
  ...
  const shared = initialize
    ...
    : await replaceSharedKegParAgentState(payload, role, inventoryPlan);
  return { ...shared.data, revision: shared.revision, initialized: shared.initialized, initializedAt: shared.initializedAt, updatedAt: shared.updatedAt, updatedByRole: shared.updatedByRole };
}
...
export async function publishWeeklyPlanSnapshot({
  ...
  if (current.recommendations?.generatedAt === frozenKegPlan.generatedAt) {
    recommendations.push(...(current.recommendations.items || []).filter(item => item.prepAdditionId));
  }
...
function getSharedWeeklyUsageValues(item, displayUnit, now) {
  if (!item || clean(item.displayUnit).toLowerCase() !== displayUnit) return null;
  return getSixWeekUsage(item, now);
}

function buildLeanKegPlan({ kind, wall, currentStock, peakUsage, maxOrder }) {
  if (!peakUsage.sampleWeeks) {
    throw new Error("No recorded usage in the last eight weeks; existing recommendations were kept.");
  }
  const need = buildOperationalRecommendation({
    kind,
    wall,
    averageUsage: peakUsage.peak,
    reserve: peakUsage.peak * MINIMUM_KEG_CUSHION,
    maxOrder,
    position: buildInventoryPosition({ connected: currentStock }),
  });
  return {
    need,
    peakWeeklyKegs: peakUsage.peak,
    preThursdayForecastKegs: 0,
    projectedThursdayStockKegs: currentStock,
    preThursdayUsageSharePct: 0,
    preThursdayUsageWeeks: 0,
    preThursdayUsageSource: "current-stock",
    variabilityCushionPct: MINIMUM_KEG_CUSHION * 100,
    variabilityHistoryWeeks: peakUsage.sampleWeeks,
    variabilitySource: "eight-week-peak",
    preDeliveryRisk: false,
  };
}

export function buildRawRecommendation(tap, level, weeklyEntries, state, settings, sharedWeeklyUsageItem = null, { now = new Date() } = {}) {
  ...
  const recentWeeklyKegs = isKeg
    ...
    : [];
  const sharedKegUsage = isKeg ? getEightWeekPeakUsage(sharedWeeklyUsageItem || { history: weeklyEntries }, now, fullOunces) : null;
  ...
  if (isLiquor) {
    const sharedOunceUsage = getSharedWeeklyUsageValues(sharedWeeklyUsageItem, "oz", now);
  ...
  if (isCocktailTap(tap)) {
    ...
    if (rawMakeQty) {
      reasonParts.push(`Current stock ${currentStockKegs} kegs is below the ${targetStockKegs}-keg target: highest week ${cocktailForecast.peakWeeklyKegs} kegs in the last 8 weeks plus 25%.`);
    } else {
      reasonParts.push(`Current stock ${currentStockKegs} kegs covers the ${targetStockKegs}-keg target: highest week in the last 8 weeks plus 25%.`);
    }
  ...
  if (rawOrderQty) {
    reasonParts.push(`Current stock ${currentStockKegs} kegs is below the ${targetStockKegs}-keg target: highest week ${beerForecast.peakWeeklyKegs} kegs in the last 8 weeks plus 25%.`);
  } else {
    reasonParts.push(`Current stock ${currentStockKegs} kegs covers the ${targetStockKegs}-keg target: highest week in the last 8 weeks plus 25%.`);
  }
