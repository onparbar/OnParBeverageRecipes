import { digestRequest, getConfig, openProductEditForm } from './pmb-product-management.mjs';
...
export function parseReportHistoryRequest(params) {
  ...
  if (!['catalog', 'menu', 'products'].includes(view)) throw Object.assign(new Error('Choose catalog, menu, or products.'), { status: 422 });
...
export async function readPmbReportHistory(input, { config = getConfig(), readPage = digestRequest, readProduct = openProductEditForm } = {}) {
  ...
  const path = input.view === 'catalog' ? '/pages/products' : '/pages/tapconfig';
  const result = await readPage(config, 'GET', path, Buffer.alloc(0), {}, new Map());
  ...
  return input.view === 'menu' ? { links: described.links } : described;
}
