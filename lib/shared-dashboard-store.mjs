...
function normalizeCompleteData(value, { requireEveryField = false } = {}) {
  ...
  for (const [group, fields] of Object.entries(FIELD_SHAPES)) {
    ...
    for (const [field, expectedShape] of Object.entries(fields)) {
      if (requireEveryField && !Object.hasOwn(groupValue, field)) {
        throw storeError(
          "INVALID_SHARED_STATE",
...
function normalizeRow(row) {
  ...
  let data;
  try {
    data = normalizeCompleteData(row.data || {}, { requireEveryField: initialized });
  } catch (error) {
