...
function normalizeData(value, { requireEveryField = false } = {}) {
  ...
  const allowed = new Set([...OBJECT_FIELDS, "recommendations", "inputEditState", "coolerEstimateState"]);
...
export function createSharedKegParAgentStore({ env = process.env, fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
  ...
  async function read({ rolloverWeek = true } = {}) {
    ...
    // First adoption keeps existing counts. Subsequent weeks reset count values,
    // including on-deck count keys, without touching assignments or snapshots.
    let data = {
      ...current.data,
      onHandOverrides: priorWeek
        ? Object.fromEntries(Object.keys(current.data.onHandOverrides).map((key) => [key, "0"]))
        : current.data.onHandOverrides,
  ...
  return Object.freeze({ read, editInputs, observeKegChanges, initialize: (payload, role) => save({ ...payload, initialize: true }, role), replace: (payload, role) => save(payload, role) });
}
