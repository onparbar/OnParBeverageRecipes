...
export function getFirstPourWindow(startDate, endDate, now = new Date()) {
  ...
  if (end <= start || end - start > 31 * DAY_MS || end > today + DAY_MS) {
    throw reportError("Search 1-31 days at a time, with no future dates.");
  }
  return { start, end };
}
...
export function summarizeFirstPourTransactions(rows, target) {
  ...
  for (const row of rows) {
    ...
    if (timestampSamples.length < 6) {
      const fields = Object.fromEntries(Object.entries(row).filter(([key, value]) => (
        /^(?:time|date|timestamp|datetime|time_stamp|transaction_time|transaction_date|transaction_datetime|start_time|end_time|poured_at|created_at|occurred_at)$/i.test(key)
  ...
  return { matchedRows, physicalTapRows, productOnlyRows, volumeOz, timestampSamples, transactionFieldNames };
}

// Find the first positive business day without depending on undocumented
// transaction timestamp fields. Every query is a read of a bounded date range.
export async function findFirstPourInRange({ target, startDate, endDate, readTransactions, now = new Date() }) {
  ...
  async function read(start, end) {
    ...
    requests += 1;
    const rows = await readTransactions({ start_time: pmbLocalMidnight(start), end_time: pmbLocalMidnight(end) });
    const summary = summarizeFirstPourTransactions(rows, target);
    summaries.set(key, summary);
    return summary;
  }
  const total = await read(range.start, range.end);
  let firstRecordedPourDate = null;
  let firstDay = null;
  if (total.matchedRows) {
    let left = range.start;
    let right = range.end;
    while (right - left > DAY_MS) {
      const midpoint = left + Math.floor((right - left) / DAY_MS / 2) * DAY_MS;
      const earlier = await read(left, midpoint);
      if (earlier.matchedRows) right = midpoint;
      else left = midpoint;
    }
    firstDay = await read(left, right);
    if (!firstDay.matchedRows) {
      throw reportError("PMB's date-range reports disagreed. No first-pour date was established.", 502);
    }
    firstRecordedPourDate = new Date(left).toISOString().slice(0, 10);
  }
  return {
