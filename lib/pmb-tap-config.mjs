...
import { parsePmbKegTimestamps } from "./pmb-keg-timestamps.mjs";
  ...
  return new Promise((resolve, reject) => {
    const request = http.request(
      ...
      (response) => {
        ...
        response.on("aborted", () => reject(Object.assign(new Error("PMB tap configuration response was interrupted."), { code: "ECONNRESET" })));
...
export async function getTapConfigRows(config, { timeoutMs = 15000, cookieJar = new Map() } = {}) {
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const page = await getDigestPage(config, "/pages/tapconfig", { cookieJar, timeoutMs });
    if (page.status !== 200) {
      if (attempt === 0 && (page.status === 0 || page.status >= 500)) continue;
      throw new Error(`PMB tap configuration read failed (${page.status || 0}).`);
    }
    const rows = parseTapConfigRows(page.raw);
    try {
      buildVerifiedKegSlotMap(rows);
      return rows;
    } catch (error) {
...
export async function getKegTappedOnRows(config, { timeoutMs = 5000, cookieJar = new Map() } = {}) {
  let page = await getDigestPage(config, "/pages/keglevel", { timeoutMs, cookieJar });
  // Retry only a failed read, never a configuration or product write.
  if (page.status === 0 || page.status >= 500) {
    page = await getDigestPage(config, "/pages/keglevel", { timeoutMs, cookieJar });
  }
  if (page.status !== 200) throw new Error("PMB keg history is unavailable.");
  const rows = parsePmbKegTimestamps(page.raw);
  if (!rows.length) throw new Error("PMB keg history did not contain readable timestamps.");
  return rows;
}
