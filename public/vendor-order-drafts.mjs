...
function buildDraftLine(item, vendor, sourceDate) {
  ...
  return {
    ...
    avgWeeklyOunces: Math.max(0, Number(item.avgWeeklyOunces) || 0),
    onHand: Math.max(0, Number(item.onHand) || 0),
    cocktailPrepRequiredBottles: Math.max(0, Number(item.cocktailPrepRequiredBottles) || 0),
    cocktailPrepShortageUnits: Math.max(0, Number(item.cocktailPrepShortageUnits) || 0),
    neededBeforeThursday: Boolean(item.neededBeforeThursday),
    neededBeforeNextDelivery: Boolean(item.neededBeforeNextDelivery),
    requiredBeforeThursday: Boolean(item.requiredBeforeThursday),
    requiredBeforeNextDelivery: Boolean(item.requiredBeforeNextDelivery),
