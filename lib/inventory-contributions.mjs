...
export function findCatalogItem(catalog, { id = "", name = "" } = {}) {
...
export async function planReceiptInventoryContributions(tracking, vendorId) {
  ...
  (vendor.items || []).forEach((line) => {
    ...
    if (!item) {
      if (!/beer keg/i.test(line.lineType)) unmatched.push(unmatchedEntry(line));
      return;
    }
...
export async function planPrepInventoryContributions({ target, generatedAt, completed, actualQuantity } = {}) {
  ...
  if (target?.kind === "liquor-refill") {
    ...
    if (!item) return normalizePlan({ sources: [], unmatched: [unmatchedEntry(target)] });
  ...
  const inventoryIngredients = recipe.ingredients.filter(isInventoryRecipeIngredient);
  ...
  inventoryIngredients.forEach((ingredient) => {
    ...
    if (!item) {
      unmatched.push(unmatchedEntry({ name: ingredient.name }));
      return;
    }
    const units = packageUnits(ingredient, item) * scale * batches;
    if (!(units > 0)) {
      unmatched.push(unmatchedEntry(item, "The package size needed for this ingredient is unavailable."));
      return;
    }
    const current = byItem.get(item.id) || { id: item.id, quantity: 0, baseline: item.baseline };
  ...
  if (!byItem.size && !unmatched.length) unmatched.push(unmatchedEntry(target, "No inventory ingredients were found for this recipe."));
  return normalizePlan({
    sources: [{
      sourceId,
