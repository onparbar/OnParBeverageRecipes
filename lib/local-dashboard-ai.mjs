...
Levels also retain firstRecordedPourAt and product swap history even when no New badge is shown. The badge's Added date means the earliest product pour found in searched PMB reports, not a verified first pour on that physical tap. Read firstPourHistoryScope, firstPourSearchedThrough and firstPourHasCoverageGaps before interpreting it. productChangedAt and previousProduct describe a separate assignment change; productChangeSource distinguishes a confirmed change from a detected one. Never substitute a refill date or earliest product record for a confirmed tap-assignment date.
