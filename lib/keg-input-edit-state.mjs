...
export function applyKegInputEdits(data, edits, { now, fallbackAt, role, physicalCount = true }) {
  ...
  for (const edit of edits) {
    ...
    metadata.clocks[key] = { id: edit.id, editedAt: new Date(time).toISOString(),
      countedAt: physicalCount ? new Date(time).toISOString()
        : previousClock?.countedAt || previousClock?.editedAt || metadata.startedAt || fallbackAt };
  }
...
export function preserveKegInputEditHistory(current, next, { now, fallbackAt, role, revision, physicalCount = true }) {
  const edits = [];
  ...
  const recorded = applyKegInputEdits(current, edits, { now, fallbackAt, role, physicalCount });
