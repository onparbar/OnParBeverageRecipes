...
const DEFAULT_VARIABILITY_CUSHION = 0.1;
...
function getVariabilityProfile(entries) {
  ...
  if (positiveForecastErrors.length) {
    return {
      cushion: clamp(percentile(positiveForecastErrors, 90), 0, MAX_VARIABILITY_CUSHION),
  ...
  if (values.length >= MIN_PROFILE_WEEKS) {
    ...
    const positiveDeviations = mean > 0
      ...
      : [];
    return {
      cushion: clamp(percentile(positiveDeviations, 90), 0, MAX_VARIABILITY_CUSHION),
      sampleWeeks: values.length,
