...
export function applyCoolerEstimateObservations(data, observations, { observedAt, fallbackAt } = {}) {
  ...
  for (const item of observations) {
    ...
    const reference = recommendations.find((entry) => Number(entry.tapNumber) === Number(item.tapNumber));
    ...
    const validReference = reference?.isKegTap && !reference.isLiquorTap && reference.actionType === "order" && key && cooler;
    const deck = validReference ? next.onDeckOverrides?.[key] : null;
    const deckMatches = deck && typeof deck === "object" && !/liquor|cocktail/i.test(clean(deck.kind))
      && clean(deck.onHandUnit) !== "oz" && nameKey(deck.name) === nameKey(item.name)
