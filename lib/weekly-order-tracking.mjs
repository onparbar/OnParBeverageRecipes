...
function normalizeAdjustmentCatalogItem(item = {}, currentPlanQuantity = 0) {
  ...
  const vendorProductName = clean(item.vendorProductName || item.productName || item.orderProductName || item.name);
  ...
  return {
    ...
    orderable: Boolean(vendor && vendorSku && name && Number.isFinite(unitCost) && unitCost > 0),
  };
}
...
export function buildWeeklyOrderTracking(recommendations = {}, now = new Date()) {
  ...
  const orderModel = buildVendorOrderDrafts(snapshot.plan, {
    generatedAt: clean(recommendations.generatedAt),
    sourceDate: clean(snapshot.publishedAt),
...
export function applyWeeklyOrderTrackingUpdate(
  ...
  if (["set-order-adjustment", "remove-order-adjustment"].includes(payload.action)) {
    ...
    const catalogItem = current.adjustmentCatalog.find((item) => item.catalogId === clean(payload.catalogId));
    if (!catalogItem || clean(payload.vendor) !== catalogItem.vendor) {
    ...
    if (payload.action === "remove-order-adjustment") {
      ...
      const reason = clean(payload.reason).slice(0, MAX_ADJUSTMENT_REASON_LENGTH);
      if (!catalogItem.orderable) throw new WeeklyOrderTrackingError("ORDER_ADJUSTMENT_PRODUCT_BLOCKED", "This product needs a current SKU and price before it can be added.", 409);
      if (!Number.isInteger(quantity) || quantity <= 0 || quantity > 999) {
        throw new WeeklyOrderTrackingError("ORDER_ADJUSTMENT_QUANTITY_INVALID", "Enter a whole order quantity from 1 to 999.");
      }
    ...
    Object.entries(orderDrafts).forEach(([id, draft]) => {
      if (clean(draft.generatedAt) === current.generatedAt && clean(draft.vendor) === catalogItem.vendor) delete orderDrafts[id];
    });
  } else if (["create-draft", "approve-draft"].includes(payload.action)) {
  ...
  return {
    ...recommendations,
    weeklyOrderTracking: { vendors, receipts, orderDrafts, orderAdjustments },
  };
}
