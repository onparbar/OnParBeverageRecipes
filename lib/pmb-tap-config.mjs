...
export async function getKegTappedOnRows(config, { timeoutMs = 5000 } = {}) {
  const page = await getDigestPage(config, "/pages/keglevel", { timeoutMs });
  if (page.status !== 200) throw new Error("PMB keg history is unavailable.");
  return parsePmbKegTimestamps(page.raw);
}
