...
async function readPmbRoute(loadRoute, label) {
  ...
  try {
    const read = async () => {
      ...
      const response = await route.GET(label === "Keg levels"
        ? new Request("http://localhost/api/keg-levels?checkSource=repair-verification") : undefined);
    ...
    // A timeout does not cancel a route handler. Reuse its pending request
    // until it settles rather than start overlapping PMB reads on each retry.
    if (!pendingReads.has(label)) {
      const pending = read();
      pendingReads.set(label, pending);
      void pending.then(() => {
        if (pendingReads.get(label) === pending) pendingReads.delete(label);
      }, () => {
        if (pendingReads.get(label) === pending) pendingReads.delete(label);
      });
    }
    return await Promise.race([
      pendingReads.get(label),
      new Promise((resolve) => {
...
export function startPmbMorningRepairRuntime({ env = process.env } = {}) {
  ...
  return startPmbMorningRepairScheduler({
    env,
    prepare: ({ beforeWrite }) => activateQueuedPmbProducts({ env, beforeWrite }),
