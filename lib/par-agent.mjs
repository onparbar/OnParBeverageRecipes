...
export function buildRawRecommendation(tap, level, weeklyEntries, state, settings, sharedWeeklyUsageItem = null) {
  ...
  if (isLiquor) {
    ...
    const suggestedRefillQty = currentStockOunces < targetStockOunces ? 1 : 0;
    const weeksOfStock = avgWeeklyOunces > 0 ? round(currentStockOunces / avgWeeklyOunces, 2) : 99;
    const reason = suggestedRefillQty
      ? `Current keg ${currentStockOunces} oz is below ${targetStockOunces} oz: ${avgWeeklyOunces} oz/week plus 100 oz. Refill purchasing is deferred until refill and cabinet inventory netting is implemented; review manually.`
    ...
    return {
      ...
      suggestedPar: "",
      suggestedRefillQty,
      deferredQty: suggestedRefillQty,
      rawOrderQty: suggestedRefillQty,
      orderQty: 0,
      orderProductName: tap.name,
      actionType: suggestedRefillQty ? "review" : "none",
      deferredReview: Boolean(suggestedRefillQty),
      deferredReason: suggestedRefillQty ? "liquor-refill-netting" : "",
      onDeckProduct: null,
...
export function getOnHandCoverage(currentTaps, onHandOverrides = {}) {
  ...
  const missingTaps = requiredTaps.filter((tap) => {
    if (!Object.hasOwn(onHandOverrides || {}, tap.key)) return true;
    const rawValue = onHandOverrides[tap.key];
    if (rawValue == null || clean(rawValue) === "") return true;
...
export async function runParAgentUpdate({ dryRun = false, patch = null, expectedRevision, role = "owner" } = {}) {
  ...
  const deferredLiquorRefillItems = items.filter((item) => item.isLiquorTap && toNumber(item.deferredQty) > 0);
  ...
  const recommendations = {
    ...
    summary: {
      ...
      deferredLiquorRefillCount: deferredLiquorRefillItems.length,
      deferredLiquorRefillTotal: deferredLiquorRefillItems.reduce((total, item) => total + toNumber(item.deferredQty), 0),
