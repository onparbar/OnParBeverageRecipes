import { getKegCountWeek } from "./keg-count-week.mjs";
import { applyKegInputEdits, preserveKegInputEditHistory } from "./keg-input-edit-state.mjs";
import { applyCoolerEstimateObservations } from "./keg-cooler-estimates.mjs";
...
function normalizeData(value, { requireEveryField = false } = {}) {
  ...
  const allowed = new Set([...OBJECT_FIELDS, "recommendations", "inputEditState", "coolerEstimateState", "inventoryOutbox"]);
  ...
  if (Object.hasOwn(value, "inventoryOutbox")) {
    if (!isObject(value.inventoryOutbox)) throw fail("INVALID_KEG_STATE", "Invalid inventory recovery queue.", 400);
    data.inventoryOutbox = clone(value.inventoryOutbox);
  }
  if (Object.hasOwn(value, "coolerEstimateState")) {
    if (!isObject(value.coolerEstimateState)) throw fail("INVALID_KEG_STATE", "Invalid cooler estimate history.", 400);
    data.coolerEstimateState = clone(value.coolerEstimateState);
  }
  if (Object.hasOwn(value, "inputEditState")) {
    if (!isObject(value.inputEditState)) throw fail("INVALID_KEG_STATE", "Invalid keg edit history.", 400);
    data.inputEditState = clone(value.inputEditState);
  }
...
export function createSharedKegParAgentStore({ env = process.env, fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
  ...
  async function read({ rolloverWeek = true } = {}) {
    ...
    if (!current.initialized || !rolloverWeek) return current;
    const week = getKegCountWeek(now());
    const priorWeek = current.data.settings.kegCountWeek;
    if (priorWeek && priorWeek >= week) return current;
    // Advance the reporting week without clearing live stock. Monday's physical
    // count reconciles the running estimate instead of replacing it with zeros.
    let data = {
      ...current.data,
      onHandOverrides: current.data.onHandOverrides,
      settings: { ...current.data.settings, kegCountWeek: week },
    };
    data = preserveKegInputEditHistory(current.data, data, { now: now().toISOString(), fallbackAt: current.updatedAt, role: "system", revision: current.revision + 1 });
    const updated = (await request("PATCH", { revision: current.revision, initialized: true }, {
      revision: current.revision + 1, data, updated_at: now().toISOString(),
      updated_by_role: "system",
    })).find((entry) => entry?.id === STATE_ID);
    if (updated) return normalizeRow(updated);
    const latest = (await request("GET")).find((entry) => entry?.id === STATE_ID);
  ...
  async function save({ expectedRevision, data, initialize = false }, role = "owner", inventoryPlan = null) {
    ...
    data = { ...data };
    delete data.coolerEstimateState;
    delete data.inventoryOutbox;
    const inventoryOutbox = clone(current.data.inventoryOutbox || { pending: [] });
    if (inventoryPlan?.sources?.length) {
      if (inventoryOutbox.pending.length >= 100) throw fail("INVENTORY_RECOVERY_BUSY", "Inventory recovery must finish before another checklist update can be saved.", 503);
      inventoryOutbox.pending.push({ id: randomUUID(), plan: clone(inventoryPlan), role, queuedAt: timestamp });
    }
    data = {
      ...data,
      inventoryOutbox,
      // Estimate bookkeeping belongs to the server, not a browser's stale copy.
      ...(current.data.coolerEstimateState ? { coolerEstimateState: current.data.coolerEstimateState } : {}),
      settings: { ...data?.settings, kegCountWeek: current.data.settings.kegCountWeek || getKegCountWeek(now()) },
    };
    data = preserveKegInputEditHistory(current.data, data, { now: timestamp, fallbackAt: current.updatedAt || timestamp, role, revision: revision + 1, physicalCount: !inventoryPlan });
  ...
  async function editInputs(edits, role = "owner") {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const current = await read();
      if (!current.initialized) throw fail("KEG_STATE_NOT_INITIALIZED", "Import Keg Levels before counting.", 409);
      let data;
      try { data = applyKegInputEdits(current.data, edits, { now: now().toISOString(), fallbackAt: current.updatedAt, role }); }
      catch (error) { throw fail("INVALID_KEG_INPUT_EDIT", error.message, 400); }
      if (JSON.stringify(data) === JSON.stringify(current.data)) return current;
      try {
        return await cas(current.revision, true, { revision: current.revision + 1, data: normalizeData(data, { requireEveryField: true }), updated_at: now().toISOString(), updated_by_role: role });
      } catch (error) {
        if (error.code !== "KEG_STATE_REVISION_CONFLICT") throw error;
      }
    }
    throw fail("KEG_STATE_BUSY", "Counts are still saved on this device. Shared storage is busy; saving can retry shortly.", 503);
  }
  async function observeKegChanges(items, observedAt) {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const current = await read();
      if (!current.initialized) return { changed: false, available: false };
      const timestamp = now().toISOString();
      let data = applyCoolerEstimateObservations(current.data, items, { observedAt, fallbackAt: current.updatedAt });
      if (JSON.stringify(data) === JSON.stringify(current.data)) return { changed: false, available: true, revision: current.revision };
      const changed = JSON.stringify(data.onHandOverrides) !== JSON.stringify(current.data.onHandOverrides)
        || JSON.stringify(data.onDeckOverrides) !== JSON.stringify(current.data.onDeckOverrides);
      data = preserveKegInputEditHistory(current.data, data, { now: timestamp, fallbackAt: current.updatedAt, role: "system", revision: current.revision + 1, physicalCount: false });
      try {
        const saved = await cas(current.revision, true, {
          revision: current.revision + 1, data: normalizeData(data, { requireEveryField: true }),
          updated_at: timestamp, updated_by_role: "system",
        });
        return { changed, available: true, revision: saved.revision };
      } catch (error) {
        if (error.code !== "KEG_STATE_REVISION_CONFLICT") throw error;
      }
    }
    throw fail("KEG_STATE_BUSY", "Cooler estimates will retry on the next PMB refresh.", 503);
  }
  async function recoverInventory() {
    const token = randomUUID();
    const { applyInventoryContributionPlan } = await import("./inventory-contributions.mjs");
    let appliedItemCount = 0;
    let inventoryStatus = {};
    for (let pass = 0; pass < 100; pass += 1) {
      const current = await read();
      const outbox = current.data.inventoryOutbox;
      if (!current.initialized || !outbox?.pending?.length) return { appliedItemCount, pending: false, ...inventoryStatus };
      if (outbox.lease && Date.parse(outbox.lease.until) > now().getTime()) {
        throw fail("INVENTORY_RECOVERY_RUNNING", "Inventory recovery is already running; the saved update remains queued.", 503);
      }
      const entry = outbox.pending[0];
      const leased = { ...outbox, lease: { token, until: new Date(now().getTime() + 300000).toISOString() } };
      try {
        await cas(current.revision, true, { revision: current.revision + 1, data: { ...current.data, inventoryOutbox: leased }, updated_at: now().toISOString(), updated_by_role: "system" });
      } catch (error) {
        if (error.code === "KEG_STATE_REVISION_CONFLICT") continue;
        throw error;
      }
      // Stable contribution IDs make crash/restart replay safe. The lease
      // prevents a newer correction from being applied ahead of this entry.
      const result = await applyInventoryContributionPlan({ ...entry.plan,
        sources: entry.plan.sources.map((source) => ({ ...source, effectiveAt: entry.queuedAt })),
      }, entry.role);
      appliedItemCount += result.appliedItemCount || 0;
      inventoryStatus = result.warning ? { warning: result.warning, shortfalls: result.shortfalls } : {};
      let acknowledged = false;
      for (let attempt = 0; attempt < 8; attempt += 1) {
        const latest = await read();
        const queue = latest.data.inventoryOutbox;
        if (queue?.lease?.token !== token || queue.pending?.[0]?.id !== entry.id) {
          throw fail("INVENTORY_RECOVERY_LEASE_LOST", "Inventory recovery will resume automatically.", 503);
        }
        try {
          await cas(latest.revision, true, { revision: latest.revision + 1,
            data: { ...latest.data, inventoryOutbox: { pending: queue.pending.slice(1) } },
            updated_at: now().toISOString(), updated_by_role: "system" });
          acknowledged = true;
          break;
        } catch (error) {
          if (error.code !== "KEG_STATE_REVISION_CONFLICT") throw error;
      ...
      if (!acknowledged) throw fail("INVENTORY_RECOVERY_BUSY", "Inventory recovery will resume automatically.", 503);
    }
    throw fail("INVENTORY_RECOVERY_BUSY", "Remaining inventory updates are queued for automatic recovery.", 503);
  }
  return Object.freeze({ read, editInputs, observeKegChanges, recoverInventory, initialize: (payload, role) => save({ ...payload, initialize: true }, role), replace: (payload, role, inventoryPlan) => save(payload, role, inventoryPlan) });
}

export const editSharedKegParAgentInputs = (edits, role) => createSharedKegParAgentStore().editInputs(edits, role);
export const observeSharedKegChanges = (items, observedAt) => createSharedKegParAgentStore().observeKegChanges(items, observedAt);
...
export const replaceSharedKegParAgentState = (payload, role, inventoryPlan) => createSharedKegParAgentStore().replace(payload, role, inventoryPlan);
export const recoverPendingInventoryUpdates = () => createSharedKegParAgentStore().recoverInventory();
