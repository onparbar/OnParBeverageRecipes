...
import { inventorySnapshotBaseMatches } from "./inventory-snapshot-guard.mjs";
...
const MERGE_SAFE_ACTIONS = new Set(["update-field", "batch-update-fields"]);
...
export function createSharedInventoryStore({
  ...
  async function request(method, filters = {}, body) {
    ...
    try {
      ...
      if (error instanceof InventoryStateError) throw error;
      throw inventoryError(
        "INVENTORY_STATE_UNAVAILABLE",
        "Shared inventory storage is unavailable.",
        503,
  ...
  async function mutate(action, payload = {}, role = "owner", { expectedRevision } = {}) {
    if (action === "save-snapshot" && payload.reliableCapture === true && payload.snapshotBaseState) {
      const revision = normalizeRevision(expectedRevision);
      const base = payload.snapshotBaseState;
      if (base.revision !== revision) {
        throw inventoryError("INVALID_INVENTORY_STATE_REVISION", "Snapshot inputs do not match the expected inventory revision.", 400);
      }
      let lastConflict = null;
      for (let attempt = 0; attempt < MAX_WRITE_ATTEMPTS; attempt += 1) {
        const current = await read();
        const captureId = String(payload.captureId || "").trim();
        if (captureId && current.snapshots.some((snapshot) => (
          snapshot?.captureMetadata?.captureId === captureId
        ))) {
          // The database may have committed a prior attempt even when its HTTP
          // response was lost. Treat an exact retry as success instead of
          // turning the completed snapshot into a revision conflict.
          return current;
        }
        if (!inventorySnapshotBaseMatches(base, current)) {
          throw inventoryError(
            "INVENTORY_STATE_REVISION_CONFLICT",
            "Inventory counts, settings, or saved snapshots changed. Review the current inventory before saving the plan.",
            409,
            { expectedRevision: revision, currentRevision: current.revision },
          );
        }
        const timestamp = now();
        const next = applyInventoryStateAction(current, action, {
          ...payload,
          captureMetadata: {
            ...payload.captureMetadata,
            sourceRevisions: { ...payload.captureMetadata?.sourceRevisions, inventory: current.revision },
          },
        }, role, timestamp);
        try {
...
export function mutateSharedInventoryState(action, payload, role = "owner", options = {}) {
  return createSharedInventoryStore().mutate(action, payload, role, options);
}
