...
export function applyCoolerEstimateObservations(data, observations, { observedAt, fallbackAt } = {}) {
  ...
  for (const item of observations) {
    ...
    const eventId = `${slot}:${item.tappedOn}`;
    const recoverableIndex = item.inventoryReference ? estimate.events.findIndex((event) =>
      event.id === eventId && event.outcome === "unmatched-product" && event.deduction === 0) : -1;
    // Unchanged, old, or out-of-order observations cannot consume stock twice.
    if (previous && (Date.parse(previous.observedAt) > observedTime || tappedAt < previous.tappedAt
      || (tappedAt === previous.tappedAt && recoverableIndex < 0))) continue;
    const references = recommendations.filter((entry) => !entry.prepAdditionId
      && Number(entry.tapNumber) === Number(item.tapNumber));
    const reference = item.inventoryReference || (references.length === 1 ? references[0] : null);
    ...
    const currentMatches = validReference && (item.inventoryReference
      ? Number(previous?.plu) === Number(item.plu) && nameKey(previous?.name) === nameKey(item.name)
      : (Number(reference.plu) === Number(item.plu)
    ...
    if (validReference && (deckMatches || currentMatches)) {
      ...
      const countTime = Date.parse(clock?.countedAt || clock?.editedAt || next.inputEditState?.startedAt || fallbackAt || estimate.startedAt);
    ...
    const event = {
      id: eventId, slot, ...observation,
      estimated: true, deduction, before, after, outcome,
    };
    if (recoverableIndex >= 0) {
      const original = estimate.events[recoverableIndex];
      estimate.events[recoverableIndex] = { ...event,
        firstObservedAt: original.firstObservedAt || original.observedAt,
        recoveredAt: observedAt, previousOutcome: original.outcome };
    } else estimate.events.push(event);
  }
