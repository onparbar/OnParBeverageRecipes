...
export function normalizePmbLevelSnapshot(value) {
  ...
  const items = value.items.map((item) => {
    ...
    const unconfiguredZero = fillLevelPercent === 0
      && (!(finiteOrNull(item.rawKegSize) > 0)
        || finiteOrNull(item.rawKegSizeDp) === null
        || Number(item.rawKegSizeDp) < 0 || Number(item.rawKegSizeDp) > 6);
    return {
      slotKey: String(item.slotKey || "").trim(),
      plu: positiveInteger(item.plu, "PLU"),
      name: String(item.name || "").trim(),
      fillLevelPercent: unconfiguredZero ? null : fillLevelPercent,
      levelAvailable: !unconfiguredZero,
      levelError: unconfiguredZero ? "PMB returned zero without valid keg configuration. Level unavailable." : "",
      ...
      tappedOn: String(item.tappedOn || "").trim().slice(0, 40),
      tappedOnCached: item.tappedOnCached === true,
      productHistory: item.productHistory ? {
        firstSeenAt: String(item.productHistory.firstSeenAt || "").slice(0, 40),
        changedAt: String(item.productHistory.changedAt || "").slice(0, 40),
        lastSeenAt: String(item.productHistory.lastSeenAt || "").slice(0, 40),
        source: ["baseline", "detected", "confirmed"].includes(item.productHistory.source) ? item.productHistory.source : "baseline",
        previousName: String(item.productHistory.previousName || "").slice(0, 240),
        introducedAt: String(item.productHistory.introducedAt || "").slice(0, 40),
        introductionSource: ["baseline", "detected", "confirmed"].includes(item.productHistory.introductionSource) ? item.productHistory.introductionSource : "baseline",
        introductionPreviousName: String(item.productHistory.introductionPreviousName || "").slice(0, 240),
      } : null,
      volumeUnit: String(item.volumeUnit || ""),
      volumeUnitDp: finiteOrNull(item.volumeUnitDp) ?? 0,
      rawPercent: unconfiguredZero ? null : finiteOrNull(item.rawPercent),
  ...
  items.forEach((item) => {
    ...
    deviceLevels[key].push({
      ...
      levelAvailable: item.levelAvailable,
      levelError: item.levelError,
