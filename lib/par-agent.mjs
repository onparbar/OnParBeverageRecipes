...
import { MINIMUM_KEG_CUSHION } from "../public/keg-demand-policy.mjs";
...
} from "../public/operations-truth-model.mjs";
...
const DEFAULT_PRE_THURSDAY_USAGE_SHARE = 3 / 7;
const DEFAULT_VARIABILITY_CUSHION = MINIMUM_KEG_CUSHION;
const MAX_VARIABILITY_CUSHION = 0.5;
const MIN_PROFILE_WEEKS = 3;
...
export async function publishWeeklyPlanSnapshot({
  ...
  const recommendations = netRollingLiquorTapRecommendations(frozenKegPlan.items.map((item) => {
...
function getPreThursdayUsageProfile(entries) {
  const shares = entries
    .map((entry) => {
      const fullWeekOz = Number(entry?.volumeOz);
      const preThursdayOz = Number(entry?.preThursdayVolumeOz);
      if (!Number.isFinite(fullWeekOz) || fullWeekOz <= 0 || !Number.isFinite(preThursdayOz) || preThursdayOz < 0) {
        return null;
      }
      return clamp(preThursdayOz / fullWeekOz, 0, 1);
    })
    .filter((value) => value !== null);

  return {
    share: shares.length >= MIN_PROFILE_WEEKS ? averageValues(shares) : DEFAULT_PRE_THURSDAY_USAGE_SHARE,
    sampleWeeks: shares.length,
    source: shares.length >= MIN_PROFILE_WEEKS ? "tap-history" : "default",
  };
}

function getVariabilityProfile(entries) {
  const values = entries
    .map((entry) => Number(entry?.value))
    .filter((value) => Number.isFinite(value) && value >= 0);
  const positiveForecastErrors = [];

  values.forEach((actual, index) => {
    const olderValues = values.slice(index + 1, index + 1 + ROLLING_USAGE_WEEKS);
    if (olderValues.length < MIN_PROFILE_WEEKS) return;
    const forecast = averageValues(olderValues);
    if (forecast <= 0) return;
    positiveForecastErrors.push(Math.max(0, (actual - forecast) / forecast));
  });

  if (positiveForecastErrors.length) {
    return {
      cushion: clamp(percentile(positiveForecastErrors, 90), MINIMUM_KEG_CUSHION, MAX_VARIABILITY_CUSHION),
      sampleWeeks: values.length,
      source: "forecast-error",
    };
  }

  if (values.length >= MIN_PROFILE_WEEKS) {
    const mean = averageValues(values);
    const positiveDeviations = mean > 0
      ? values.map((value) => Math.max(0, (value - mean) / mean))
      : [];
    return {
      cushion: clamp(percentile(positiveDeviations, 90), MINIMUM_KEG_CUSHION, MAX_VARIABILITY_CUSHION),
      sampleWeeks: values.length,
      source: "recent-variation",
    };
  }

  return {
    cushion: DEFAULT_VARIABILITY_CUSHION,
    sampleWeeks: values.length,
    source: "default",
  };
}

function buildLeanKegPlan({ kind, wall, averageUsage, currentStock, entries, maxOrder }) {
  const preThursdayProfile = getPreThursdayUsageProfile(entries);
  const variabilityProfile = getVariabilityProfile(entries);
  const preThursdayForecast = averageUsage * preThursdayProfile.share;
  const projectedThursdayStock = Math.max(0, currentStock - preThursdayForecast);
  const variabilityReserve = averageUsage * variabilityProfile.cushion;
  const need = buildOperationalRecommendation({
    kind,
    wall,
    averageUsage,
    reserve: variabilityReserve,
    maxOrder,
    position: buildInventoryPosition({ connected: projectedThursdayStock }),
  });

  return {
    need,
    preThursdayForecastKegs: round(preThursdayForecast, 3),
    projectedThursdayStockKegs: round(projectedThursdayStock, 3),
    preThursdayUsageSharePct: round(preThursdayProfile.share * 100, 1),
    preThursdayUsageWeeks: preThursdayProfile.sampleWeeks,
    preThursdayUsageSource: preThursdayProfile.source,
    variabilityCushionPct: round(variabilityProfile.cushion * 100, 1),
    variabilityHistoryWeeks: variabilityProfile.sampleWeeks,
    variabilitySource: variabilityProfile.source,
    preDeliveryRisk: currentStock < preThursdayForecast,
  };
}
...
export function buildRawRecommendation(tap, level, weeklyEntries, state, settings, sharedWeeklyUsageItem = null) {
  ...
  const avgWeeklyKegs = sharedKegUsage
    ...
    : weeklyKegs.length
      ...
      : 0;
  const currentStockKegs = round(buildInventoryPosition({
    connected: liveFraction,
    onHand: backupKegs,
    onDeck: onDeckOnHandKegs,
  }).available, 3);
  ...
  if (isLiquor) {
    ...
    const liquorNeed = buildOperationalRecommendation({
      kind: "liquor",
      wall: tap.wall,
      averageUsage: avgWeeklyOunces,
      reserve: LIQUOR_RESERVE_OZ,
      bottleSize: bottleOz,
      position: buildInventoryPosition({ connected: currentStockOunces }),
    });
    const targetStockOunces = round(liquorNeed.targetStock, 2);
    const gapOunces = round(liquorNeed.gap, 2);
    const suggestedBottleOrderQty = liquorNeed.orderQuantity;
    const refillBottleQty = suggestedBottleOrderQty;
    ...
    return {
      ...
      weeklyKegs: [],
      weeklyOunces,
      usageSampleWeeks: sharedOunceUsage?.sampleWeeks ?? weeklyOunces.length,
      usageExpectedWeeks: 6,
      reason,
    };
  }
  ...
  if (isCocktailTap(tap)) {
    const { need: cocktailNeed, ...cocktailForecast } = buildLeanKegPlan({
      kind: "cocktail",
      wall: tap.wall,
      averageUsage: avgWeeklyKegs,
      currentStock: currentStockKegs,
      entries: sharedKegUsage?.entries || [],
    });
    const targetStockKegs = round(cocktailNeed.targetStock, 2);
    const gapKegs = round(cocktailNeed.gap, 3);
    const rawMakeQty = cocktailNeed.orderQuantity;
    const weeksOfStock = avgWeeklyKegs > 0 ? round(currentStockKegs / avgWeeklyKegs, 2) : 99;
    const reasonParts = [];

    if (rawMakeQty) {
      reasonParts.push(`Projected Thursday stock ${cocktailForecast.projectedThursdayStockKegs} kegs is below the ${targetStockKegs}-keg target: ${avgWeeklyKegs}/week plus a ${cocktailForecast.variabilityCushionPct}% tap cushion.`);
    } else {
      reasonParts.push(`Projected Thursday stock ${cocktailForecast.projectedThursdayStockKegs} kegs covers the ${targetStockKegs}-keg target after ${cocktailForecast.preThursdayForecastKegs} kegs of forecast use before Thursday.`);
    }
    ...
    return {
      ...
      weeksOfStock,
      weeklyKegs,
      usageSampleWeeks: sharedKegUsage?.sampleWeeks ?? weeklyKegs.length,
      usageExpectedWeeks: 6,
  ...
  const { need: beerNeed, ...beerForecast } = buildLeanKegPlan({
    kind: "beer",
    wall: tap.wall,
    averageUsage: avgWeeklyKegs,
    currentStock: currentStockKegs,
    entries: sharedKegUsage?.entries || [],
    maxOrder: maxOrderPerTap,
  });
  const targetStockKegs = round(beerNeed.targetStock, 2);
  const gapKegs = round(beerNeed.gap, 3);
  const calculatedOrderQty = beerNeed.uncappedOrderQuantity;
  const rawOrderQty = beerNeed.orderQuantity;
  const orderCapApplied = beerNeed.orderCapApplied;
  const weeksOfStock = avgWeeklyKegs > 0 ? round(currentStockKegs / avgWeeklyKegs, 2) : 99;
  ...
  if (rawOrderQty) {
    reasonParts.push(`Projected Thursday stock ${beerForecast.projectedThursdayStockKegs} kegs is below the ${targetStockKegs}-keg target: ${avgWeeklyKegs}/week plus a ${beerForecast.variabilityCushionPct}% tap cushion.`);
  } else {
    reasonParts.push(`Projected Thursday stock ${beerForecast.projectedThursdayStockKegs} kegs covers the ${targetStockKegs}-keg target after ${beerForecast.preThursdayForecastKegs} kegs of forecast use before Thursday.`);
  }
  if (rawOrderQty && onDeckProduct?.name) {
    reasonParts.push(`Order ${onDeckProduct.name} from On Deck instead of ${tap.name}.`);
  }
  if (calculatedOrderQty > maxOrderPerTap) {
    reasonParts.push(`Review this larger order: ${calculatedOrderQty} kegs exceeds the ${maxOrderPerTap}-keg review threshold. The full rounded quantity is retained to cover the shortage and variability cushion.`);
  }
...
export async function runParAgentUpdate({ dryRun = false, patch = null, expectedRevision, role = "owner" } = {}) {
  ...
  const usageRequiredTaps = snapshot.currentTaps.filter(isWeeklyUsageRequiredItem);
  const missingUsageTaps = usageRequiredTaps
  ...
  const staleUsageTaps = getMissingLatestCompletedUsageTaps(
    usageRequiredTaps.map((tap) => weeklyUsageByTap.get(toNumber(tap.tapNumber))),
  );
