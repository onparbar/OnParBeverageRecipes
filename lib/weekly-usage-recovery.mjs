...
import { getWeeklyUsageSource, getPmbWeeklyUsageRange } from "../public/pmb-weekly-usage-policy.mjs";
...
function needsRecovery(state, week) {
  return state?.initialized === true && (state.data?.activeItems || []).some((item) => {
    if (item.hidden || !(Number(item.tapNumber) > 0)) return false;
    const introduced = getTapAssignmentUsageStart(item);
    if (introduced && introduced > week.endDate) return false;
    const entry = (state.data.historyOverrides?.[item.id] || []).find((entry) => entry.label === week.label)
      || (item.history || []).find((entry) => entry.label === week.label);
    // A saved unknown is not a completed recovery. Retry with new evidence,
    // retaining the existing throttling and never replacing a reviewed value.
    return getWeeklyUsageSource(entry) !== "PMB" || !isUsableWeeklyUsageEntry(entry);
  });
}
...
function mergeEntry(history, incoming) {
  const sameWeek = entry => getPmbWeeklyUsageRange(entry.label)?.startTime === getPmbWeeklyUsageRange(incoming.label)?.startTime;
  const existing = history.find(sameWeek);
  // Completed, reviewed history is not a target of automatic recovery.
  if (existing && (getWeeklyUsageSource(existing) === "PMB" && (isUsableWeeklyUsageEntry(existing) || existing.zeroUsageVerified)
    || getWeeklyUsageSource(existing) === "CSV" && !isUsableWeeklyUsageEntry(incoming)
    || !getWeeklyUsageSource(existing))) return history;
  return [...history.filter(entry => !sameWeek(entry)), incoming]
