...
import { isWeeklyCountOnlyInventoryItem } from "../public/inventory-count-policy.mjs";
...
function sanitizeGroupOverrides(value) {
  return Object.fromEntries(Object.entries(sanitizeOverrideMap(value))
    .filter(([, group]) => ["Liquor Cabinet", "Mixer Cabinet", "Other"].includes(group)));
}

function sanitizeCountedItemsAt(value) {
  return Object.fromEntries(Object.entries(value && typeof value === "object" ? value : {})
    .slice(0, MAX_ITEMS)
    .map(([id, timestamp]) => [cleanId(id), clean(timestamp, 40)])
    .filter(([id, timestamp]) => id && Number.isFinite(Date.parse(timestamp))));
}
...
function sanitizeInventoryContributions(value) {
  ...
  return Object.fromEntries(Object.entries(value).slice(0, MAX_INVENTORY_CONTRIBUTIONS).flatMap(([key, entry]) => {
    ...
    return [[clean(key, 360) || `${sourceId}::${itemId}`, {
      ...
      balanceVersion: entry?.balanceVersion === 1 ? 1 : 0,
...
function sanitizeContributionShortfalls(value) {
  return Object.fromEntries(Object.entries(value && typeof value === "object" ? value : {})
    .slice(0, MAX_ITEMS)
    .map(([id, amount]) => [cleanId(id), Math.round(Math.max(0, finiteNumber(amount)) * 1000) / 1000])
...
function sanitizeSnapshotSummary(summary) {
  ...
  return {
    ...
    ...(summary.simpleSyrupNeed && typeof summary.simpleSyrupNeed === "object" ? {
      simpleSyrupNeed: {
        totalOz: Math.max(0, finiteNumber(summary.simpleSyrupNeed.totalOz)),
        gallons: Math.max(0, finiteNumber(summary.simpleSyrupNeed.totalOz)) / 128,
        complete: summary.simpleSyrupNeed.complete === true,
      },
    } : {}),
...
function sanitizeCaptureMetadata(value, savedAt, role, weekOf) {
  ...
  return {
    captureId: clean(value.captureId, 120),
...
function sanitizeKegPlanTapInput(item) {
  ...
  return {
    ...
    avgWeeklyKegs: Math.max(0, finiteNumber(item.avgWeeklyKegs)),
    avgWeeklyOunces: Math.max(0, finiteNumber(item.avgWeeklyOunces)),
    variabilityCushionPct: item.variabilityCushionPct == null ? null : Math.min(50, Math.max(0, finiteNumber(item.variabilityCushionPct))),
    preThursdayUsageSharePct: item.preThursdayUsageSharePct == null ? null : Math.min(100, Math.max(0, finiteNumber(item.preThursdayUsageSharePct))),
    inventoryStateMissing: Boolean(item.inventoryStateMissing),
  };
}
...
function sanitizeSnapshot(snapshot) {
  ...
  return {
    ...
    ...(Array.isArray(snapshot.orders) ? { orders: snapshot.orders.map(normalizeSnapshotOrder).filter(Boolean).slice(0, 30) } : {}),
...
export function normalizeInventoryState(value) {
  ...
  return {
    ...
    current: {
      ...
      groupOverrides: sanitizeGroupOverrides(value.current?.groupOverrides),
      ...
      contributionShortfalls: sanitizeContributionShortfalls(value.current?.contributionShortfalls),
      countedAt: clean(value.current?.countedAt, 40),
      countedItemsAt: sanitizeCountedItemsAt(recoverLegacyInventoryCountReceipts(value)),
      countEvidenceVersion: 1,
...
function getInventoryCountedAt(payload, now) {
  const countedTime = Date.parse(payload?.countedAt || "");
  return Number.isFinite(countedTime) && countedTime <= now.getTime()
    ? new Date(countedTime).toISOString()
    : now.toISOString();
}
...
export function applyInventoryStateAction(
  ...
  switch (action) {
    case "initialize": {
      ...
      state.current.groupOverrides = sanitizeGroupOverrides(payload.groupOverrides);
      state.current.speechAliases = sanitizeSpeechAliases(payload.speechAliases);
      state.current.inventoryContributions = {};
      state.current.contributionShortfalls = {};
    ...
    case "update-field": {
      ...
      if (field === "onHand") {
        delete state.current.contributionShortfalls[normalizedId];
        // Keep source receipts so a retry cannot reapply stock already counted.
        if (normalizedValue === "") {
          delete state.current.countedItemsAt[normalizedId];
          state.current.countedAt = Object.values(state.current.countedItemsAt).sort().at(-1) || "";
        } else {
          state.current.countedAt = getInventoryCountedAt(payload, now);
          state.current.countedItemsAt[normalizedId] = state.current.countedAt;
        }
      }
      return markCurrentUpdated(state, role, now);
    }
    case "batch-update-fields": {
      ...
      normalizedChanges.forEach((change) => {
        ...
        // Physical counts replace the balance, not the idempotency receipts.
      });
      const countChanges = [...normalizedChanges.values()].filter((change) => change.field === "onHand");
      if (countChanges.length) {
        const countsCleared = payload.source === "clear-on-hand"
          || countChanges.every((change) => change.value === "");
        state.current.countedAt = countsCleared ? "" : getInventoryCountedAt(payload, now);
        countChanges.forEach((change) => {
          delete state.current.contributionShortfalls[change.id];
          if (countsCleared || change.value === "") delete state.current.countedItemsAt[change.id];
          else state.current.countedItemsAt[change.id] = state.current.countedAt;
        });
      }
      return markCurrentUpdated(state, role, now);
    }
    case "apply-contributions": {
      ...
      sources.forEach((source) => {
        ...
        itemIds.forEach((itemId) => {
          ...
          // Retain the amount below zero so correcting/undoing a deduction (or
          // a consumed receipt) cannot manufacture stock after display clamping.
          const shortfall = state.current.contributionShortfalls[itemId] || 0;
          const adjusted = Math.round((currentValue - shortfall + nextQuantity - priorQuantity) * 1000) / 1000;
          const effectiveAt = Date.parse(source.effectiveAt || "");
          const countedAt = Date.parse(state.current.countedItemsAt[itemId] || "");
          const coveredByCount = Number.isFinite(effectiveAt) && Number.isFinite(countedAt) && effectiveAt <= countedAt;
          const legacyCredit = prior && prior.balanceVersion !== 1 && priorQuantity < 0 && nextQuantity > priorQuantity;
          const priorUpdatedAt = Date.parse(prior?.updatedAt || "");
          const recountedSincePrior = Number.isFinite(countedAt) && Number.isFinite(priorUpdatedAt) && countedAt > priorUpdatedAt;
          // Old clamped records did not retain their shortage. Do not guess a
          // stock credit from those records; a fresh count is authoritative.
          if (legacyCredit && !coveredByCount && !recountedSincePrior) {
            throw inventoryError("INVENTORY_RECOUNT_REQUIRED",
              `Recount ${itemId.replace(/-/g, " ")} before correcting this older prep entry; its original stock deduction cannot be verified.`,
              409, { itemId });
          }
          if (!coveredByCount && !(legacyCredit && recountedSincePrior)) {
            state.current.onHandOverrides[itemId] = String(Math.max(0, adjusted));
            if (adjusted < 0) state.current.contributionShortfalls[itemId] = -adjusted;
            else delete state.current.contributionShortfalls[itemId];
          }
          Object.entries(state.current.inventoryContributions).forEach(([key, entry]) => {
          ...
          if (nextQuantity !== 0) {
            state.current.inventoryContributions[contributionKey] = {
              ...
              balanceVersion: !prior || prior.balanceVersion === 1 ? 1 : 0,
              baseline: Math.max(0, finiteNumber(next?.baseline ?? prior?.baseline)),
              reason,
              updatedAt: prior && nextQuantity === priorQuantity ? prior.updatedAt : now.toISOString(),
            };
          }
        });
      });
    ...
    case "upsert-custom": {
      ...
      if (payload.countInput === true && normalized.onHandDisplay !== "") {
        state.current.countedItemsAt[normalized.id] = getInventoryCountedAt(payload, now);
      }
      if (payload.countInput === true || normalized.onHandDisplay !== stateValue?.current?.onHandOverrides?.[normalized.id]) {
        delete state.current.contributionShortfalls[normalized.id];
      }
      return markCurrentUpdated(state, role, now);
    }
    ...
    case "reorder-items": {
      requireInitialized(state);
      if (payload.movedItem) {
        const id = cleanId(payload.movedItem.id);
        const group = clean(payload.movedItem.group);
        if (!id || !["Liquor Cabinet", "Mixer Cabinet", "Other"].includes(group)
          || !sanitizeItemOrder(payload.itemOrder).includes(id)) {
          throw inventoryError("INVALID_INVENTORY_ACTION", "A valid item and cabinet are required.");
        }
        state.current.groupOverrides[id] = group;
      }
      state.current.itemOrder = sanitizeItemOrder(payload.itemOrder);
      return markCurrentUpdated(state, role, now);
    }
    ...
    case "save-snapshot": {
      ...
      if (existingSnapshot?.orders?.some((order) => order.ordered)) {
        throw inventoryError("SNAPSHOT_ORDERS_ALREADY_PLACED", "This snapshot has placed orders and cannot be replaced.", 409);
      }
      if (reliableCapture) {
        const missingSections = getInventoryCountSections(payload.items, state.current.countedItemsAt, now)
          .filter((section) => !section.complete);
        if (missingSections.length) {
          throw inventoryError("INVENTORY_COUNTS_NOT_CURRENT",
            `This week's counts are still needed for: ${missingSections.map((section) => section.name).join(", ")}.`);
        }
      }
      const snapshot = sanitizeSnapshot({
        ...
        captureMetadata: {
          ...
          captureId: clean(payload.captureId || payload.captureMetadata?.captureId, 120),
        },
      ...
      // Preserve live stock and its event ledger when archiving the weekly count.
      // Weekly-count-only products intentionally start blank after capture so
      // an old count cannot be mistaken for the next operating week's count.
      snapshot.items.filter(isWeeklyCountOnlyInventoryItem).forEach((item) => {
        delete state.current.onHandOverrides[item.id];
        delete state.current.countedItemsAt[item.id];
      });
      state.current.countedAt = Object.values(state.current.countedItemsAt).sort().at(-1) || "";
      return markCurrentUpdated(state, role, now);
    }
    case "save-order-snapshot": {
      requireInitialized(state);
      if (role !== "owner") throw inventoryError("OWNER_REQUIRED", "Only an owner can record a placed order.", 403);
      const order = normalizeSnapshotOrder(payload.order);
      if (!order) throw inventoryError("INVALID_ORDER_SNAPSHOT", "Valid order details are required.");
      const snapshot = state.snapshots.find((entry) => entry.kegPlanSnapshot?.generatedAt === order.generatedAt);
      if (!snapshot) throw inventoryError("ORDER_SNAPSHOT_NOT_FOUND", "The inventory snapshot for this order is missing. The order was not marked complete; reload and retry.", 409);
      const orders = Array.isArray(snapshot.orders) ? snapshot.orders : [];
      const existing = orders.find((entry) => entry.vendorId === order.vendorId);
      if (existing?.ordered && order.ordered && existing.draftId !== order.draftId) {
        throw inventoryError("PLACED_ORDER_CHANGED", "The saved placed order differs from this draft. Reload before continuing.", 409);
      }
      const savedOrder = existing?.ordered && order.ordered ? existing : order;
      snapshot.orders = [...orders.filter((entry) => entry.vendorId !== order.vendorId), savedOrder];
      return markCurrentUpdated(state, role, now);
    }
    ...
    case "restore-snapshot": {
      ...
      state.current.inventoryContributions = {};
      state.current.contributionShortfalls = {};
      state.current.countedAt = "";
      state.current.countedItemsAt = {};
      return markCurrentUpdated(state, role, now);
    }
    default:
