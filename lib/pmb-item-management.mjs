import { getOwnerVerifiedPmbPortionRows, VERIFIED_PMB_PORTION_SCHEMA } from "./verified-pmb-portions.mjs";
...
function safePath(value, fallback = "/pages/items") {
  const url = new URL(clean(value) || fallback, `http://pmb.local${fallback}`);
...
function hasVerifiedItemsFormIdentity(form, item) {
  // This controller's Items editor stores quantity in the portion definition,
  // not in the item form. Only use our owner-verified identity in that case.
  if (form.controls.some((control) => /quantity|qty|volume|ounce|\boz\b/i.test(control.name)
    && !/^Product_Associated_Volume_Unit$/i.test(control.name))) return false;
  const [verified] = getOwnerVerifiedPmbPortionRows([{
    product_plu: item.productPlu,
    portion_name: item.portionName,
  }]);
  if (!verified
    || String(verified[VERIFIED_PMB_PORTION_SCHEMA.itemIdField]) !== String(item.itemId)
    || Number(verified[VERIFIED_PMB_PORTION_SCHEMA.quantityField]) !== item.quantityOz) return false;

  const exactControl = (name) => {
    const matches = form.controls.filter((control) => control.name === name && !control.submit);
    return matches.length === 1 ? matches[0] : null;
  };
  const id = exactControl("Id_Input");
  const editId = exactControl("fd_edit_id");
  const portion = exactControl("Portion_Input");
  const product = exactControl("Product_Input");
  if (!id || !editId || !portion || !product
    || clean(id.value) !== String(item.itemId)
    || clean(editId.value) !== String(item.itemId)
    || normalize(portion.value) !== normalize(item.portionName)
    || !clean(product.value)) return false;

  // The live product picker ties the displayed name to the expected PLU.
  // Never accept the product's name alone or a PLU found elsewhere on the page.
  const productLists = [...form.inner.matchAll(/<(?:ol|ul)\b[^>]*\bid=["']productList["'][^>]*>([\s\S]*?)<\/(?:ol|ul)>/gi)];
  if (productLists.length !== 1) return false;
  const catalogMatches = [...productLists[0][1].matchAll(/<li\b[^>]*>([\s\S]*?)<\/li>/gi)]
    .map((match) => ({
      name: decodeHtml((match[1].match(/<a\b[^>]*>([\s\S]*?)<\/a>/i)?.[1] || "").replace(/<[^>]*>/g, "")),
      plu: decodeHtml((match[1].match(/<cite\b[^>]*>([\s\S]*?)<\/cite>/i)?.[1] || "").replace(/<[^>]*>/g, "")),
    }))
    .filter((entry) => normalize(entry.name) === normalize(product.value));
  return catalogMatches.length === 1 && Number(catalogMatches[0].plu) === item.productPlu;
}
...
function findSaveDescriptor(html, item, expectedPortion) {
  ...
  parseForms(html).forEach((form) => {
    ...
    const quantityEvidence = form.controls.some((control) => control.name === "Portion_Input")
      ? hasVerifiedItemsFormIdentity(form, item)
      : form.controls.some((control) => (
...
const managementChallenges = new Map();
const MANAGEMENT_CHALLENGE_TTL_MS = 60_000;

function managementChallengeKey(config) {
  return crypto.createHash("sha256")
    .update(JSON.stringify([config.baseUrl, config.username, config.password]))
    .digest("hex");
}

function digestAuthorization(config, method, path, challenge, nonceCount = 1) {
...
function httpRequest(config, method, path, body = Buffer.alloc(0), headers = {}) {
  ...
  return new Promise((resolve, reject) => {
    const request = transport.request({
      ...
      method,
      agent: false,
      signal: config.readSignal,
      headers: { Accept: "text/html,*/*", "User-Agent": "OnParBeverageDashboard/1.0", ...headers, Connection: "close" },
    }, (response) => {
      const chunks = [];
      response.on("error", reject);
      response.on("aborted", () => reject(Object.assign(new Error("PMB item management response was interrupted."), { code: "ECONNRESET" })));
...
async function digestRequest(config, method, path, body, headers, cookieJar) {
  const key = managementChallengeKey(config);
  const cached = managementChallenges.get(key);
  const reusable = cached && Date.now() - cached.createdAt < MANAGEMENT_CHALLENGE_TTL_MS;
  const cookie = cookieHeader(cookieJar);
  // PMB sometimes closes new unauthenticated connections before returning a
  // challenge. Reuse a recent challenge, but always authenticate this request.
  const first = await httpRequest(config, method, path, body, {
    ...headers,
    ...(cookie ? { Cookie: cookie } : {}),
    ...(reusable ? {
      Authorization: digestAuthorization(config, method, path, cached.challenge, ++cached.nonceCount),
    } : {}),
  });
  absorbCookies(first.headers, cookieJar);
  if (first.status !== 401) return first;
  managementChallenges.delete(key);
  const challenge = parseDigestChallenge(first.headers?.["www-authenticate"]);
  if (!challenge.realm || !challenge.nonce) return first;
  const session = { challenge, createdAt: Date.now(), nonceCount: 1 };
  managementChallenges.set(key, session);
  const nextCookie = cookieHeader(cookieJar);
  // Only an explicit authentication rejection permits this second request.
  // A disconnected or uncertain price-save response is never replayed here.
  ...
  if (result.status === 401) managementChallenges.delete(key);
  return result;
}

async function readOnlyDigestRequest(config, method, path, body, headers, cookieJar) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await digestRequest(config, method, path, body, headers, cookieJar);
    } catch (error) {
      const transient = /ECONNRESET|EPIPE|ETIMEDOUT|socket hang up|timed out|interrupted/i.test(`${error.code || ""} ${error.message || ""}`);
      if (attempt < 2 && transient) {
        // Retry only read-only form discovery, never an uncertain price save.
        cookieJar.clear();
        await new Promise((resolve) => setTimeout(resolve, 500 * (attempt + 1)));
        continue;
      }
      const stage = path.split("?")[0];
      throw Object.assign(new Error(`PMB management read failed at ${method} ${stage}: ${error.message || "connection interrupted"}`), {
        code: "PMB_MANAGEMENT_READ_FAILED", status: 503, cause: error,
      });
    }
  }
}
...
// Pricing fallback can only read tap configuration or open an existing product.
// It cannot submit a save, activate a product, or send a configuration update.
export async function readPmbPricingManagementPage(config, { plu } = {}) {
  const isProduct = plu !== undefined;
  if (isProduct && (!Number.isSafeInteger(Number(plu)) || Number(plu) <= 0)) {
    fail("A verified product PLU is required to read PMB pricing.", "PMB_PRICING_IDENTITY_REQUIRED");
  }
  const path = isProduct ? "/pages/products" : "/pages/tapconfig";
  const request = isProduct
    ? encodedRequest([["fd_edit_plu", String(plu)], ["submit_edit_product", "edit"]])
    : { body: Buffer.alloc(0), headers: {} };
  const response = await readOnlyDigestRequest(config, isProduct ? "POST" : "GET", path, request.body, request.headers, new Map());
  if (response.status !== 200) {
    fail(`PMB pricing read failed at ${path} (${response.status || 0}).`, "PMB_PRICING_MANAGEMENT_UNAVAILABLE", 503);
  }
  return response.raw;
}
...
async function formRequest(config, descriptor, cookieJar) {
  ...
  if (method === "GET") {
    ...
    return readOnlyDigestRequest(config, "GET", `${url.pathname}${url.search}`, Buffer.alloc(0), {}, cookieJar);
  }
  const request = encodedRequest(entries);
  // This opens the edit form; the price-save request uses digestRequest directly.
  return readOnlyDigestRequest(config, "POST", descriptor.action, request.body, request.headers, cookieJar);
}

async function openVerifiedItemForm(config, item, expectedPortion) {
  const cookieJar = new Map();
  const list = await readOnlyDigestRequest(config, "GET", "/pages/items", Buffer.alloc(0), {}, cookieJar);
...
export async function preparePmbPortionManagementEdits(config, itemRows, request, schema) {
  ...
  const descriptors = [];
  for (const portion of request.portions) {
    ...
    descriptors.push(await openVerifiedItemForm(config, item, portion));
  }
