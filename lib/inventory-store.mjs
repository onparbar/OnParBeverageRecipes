...
function validateReliableCapture(payload, now) {
  if (!isEasternMonday(now)) {
    throw inventoryError(
      "MONDAY_SNAPSHOT_DAY_REQUIRED",
      "The Monday Inventory Snapshot can be captured at any time on Monday Eastern.",
      409,
    );
  }
...
export function applyInventoryStateAction(
  ...
  switch (action) {
    ...
    case "save-snapshot": {
      ...
      const weekOf = reliableCapture ? getEasternDate(now) : getMondayDate(now);
      const existingSnapshot = state.snapshots.find((entry) => entry.weekOf === weekOf);
      if (reliableCapture && existingSnapshot) return state;
