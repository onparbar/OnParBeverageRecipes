import { mkdir, open, readFile, readdir, rename } from "node:fs/promises";
import path from "node:path";
import { setTimeout as delay } from "node:timers/promises";
import { recordDashboardActivity } from "./dashboard-activity-log.mjs";

export const PMB_REPAIR_TIME_ZONE = "America/New_York";
const clock = new Intl.DateTimeFormat("en-CA", {
  timeZone: PMB_REPAIR_TIME_ZONE,
  year: "numeric", month: "2-digit", day: "2-digit",
  hour: "2-digit", minute: "2-digit", hourCycle: "h23",
});
const schedulerKey = Symbol.for("onpar.pmb-morning-repair.scheduler");
const repairSlots = [
  { id: "monday-reset", hour: 7, scheduledTime: "07:00", verificationUntil: "07:50", weekday: 1 },
  { id: "daily", hour: 10, scheduledTime: "10:00", verificationUntil: "10:50" },
  { id: "nightly", hour: 0, minute: 40, scheduledTime: "00:40", verificationUntil: "00:55" },
];

function slotsForDate(date) {
  const weekday = new Date(`${date}T12:00:00Z`).getUTCDay();
  return repairSlots.filter((slot) => slot.weekday === undefined || slot.weekday === weekday);
}

function currentRepairSlot(now) {
  const time = getPmbRepairClock(now);
  return slotsForDate(time.date).find((slot) => time.hour === slot.hour && time.minute === (slot.minute || 0));
}

function verificationEnded(time, slot) {
  const [hour, minute] = slot.verificationUntil.split(":").map(Number);
  return time.hour * 60 + time.minute >= hour * 60 + minute;
}

function claimFilename(date, slot) {
  // Preserve existing daily claims across upgrades and rollbacks.
  return slot.id === "daily" ? `${date}.json` : `${date}-${slot.id}.json`;
}

export function getPmbRepairClock(now = new Date()) {
  const parts = Object.fromEntries(clock.formatToParts(now).map(({ type, value }) => [type, value]));
  return { date: `${parts.year}-${parts.month}-${parts.day}`, hour: Number(parts.hour), minute: Number(parts.minute) };
}

export function isPmbMorningRepairEnabled(env = process.env) {
  return env.NODE_ENV === "production"
    && env.ONPAR_DEPLOYMENT_TARGET === "on-site"
    && env.ONPAR_PMB_REPAIR_SCHEDULER === "1"
    && String(env.PMB_MORNING_REPAIR_ENABLED || "").trim().toLowerCase() !== "false"
    && env.NEXT_PHASE !== "phase-production-build"
    && !env.VERCEL;
}

export function isPmbRepairWindow(now = new Date()) {
  return Boolean(currentRepairSlot(now));
}

export function getPmbPreparationSlot(now = new Date()) {
  const time = getPmbRepairClock(now);
  const minute = time.hour * 60 + time.minute;
  const slot = slotsForDate(time.date).find((entry) => {
    const scheduled = entry.hour * 60 + (entry.minute || 0);
    return minute >= scheduled - 5 && minute <= scheduled;
  });
  return slot ? { ...slot, date: time.date } : null;
}

function preparationFilename(date, slot) {
  return `${date}-${slot.id}-preparation.json`;
}

function createPreparationRunner({ env = process.env, now = () => new Date(),
  stateDirectory = defaultStateDirectory(), prepare } = {}) {
  let running = false;
  return async () => {
    const slot = getPmbPreparationSlot(now());
    if (running || !prepare || !slot || !isPmbMorningRepairEnabled(env)) return;
    running = true;
    const filePath = path.join(stateDirectory, preparationFilename(slot.date, slot));
    let claimed = false;
    try {
      await mkdir(stateDirectory, { recursive: true, mode: 0o700 });
      const file = await open(filePath, "wx", 0o600);
      claimed = true;
      try { await file.writeFile(JSON.stringify({ status: "preparing", startedAt: now().toISOString() })); await file.sync(); }
      finally { await file.close(); }
      const beforeWrite = async () => {
        const current = getPmbPreparationSlot(now());
        if (!isPmbMorningRepairEnabled(env) || current?.date !== slot.date || current?.id !== slot.id) {
          throw Object.assign(new Error("Product preparation window closed."), { code: "PMB_REPAIR_WINDOW_CLOSED" });
        }
      };
      const result = await prepare({ beforeWrite });
      await atomicJson(filePath, { status: result.verified ? "ready" : "needs-attention", result, finishedAt: now().toISOString() });
    } catch (error) {
      if (claimed) await atomicJson(filePath, { status: "needs-attention", code: error.code || "PMB_PREPARATION_FAILED",
        result: error.productActivation || null, finishedAt: now().toISOString() }).catch(() => {});
    } finally { running = false; }
  };
}

function defaultStateDirectory() {
  // Production releases symlink data to the persistent service directory.
  return path.join(process.cwd(), "data", "pmb-morning-repair");
}

async function atomicJson(file, value) {
  const temporary = `${file}.${process.pid}.${crypto.randomUUID()}.tmp`;
  const handle = await open(temporary, "wx", 0o600);
  try {
    await handle.writeFile(`${JSON.stringify(value, null, 2)}\n`);
    await handle.sync();
  } finally {
    await handle.close();
  }
  await rename(temporary, file);
}

async function readRunRecord(file) {
  let primaryError;
  try {
    const primary = await readJson(file);
    if (primary) return primary;
  } catch (error) {
    primaryError = error;
    // Status reporting may recover from the backup; execution claims never do.
  }
  const backup = await readJson(`${file}.backup.json`);
  if (!backup && primaryError) throw primaryError;
  return backup;
}

async function readJson(file) {
  try {
    return JSON.parse(await readFile(file, "utf8"));
  } catch (error) {
    if (error.code === "ENOENT") return null;
    throw error;
  }
}

export async function readPmbMorningRepairStatus({ env = process.env, now = new Date(), stateDirectory = defaultStateDirectory() } = {}) {
  const time = getPmbRepairClock(now);
  const base = {
    enabled: isPmbMorningRepairEnabled(env),
    timeZone: PMB_REPAIR_TIME_ZONE,
    scheduledTime: "10:00",
    schedules: repairSlots.map((slot) => ({ ...slot })),
    today: time.date,
    windowOpen: isPmbRepairWindow(now),
  };
  try {
    const slots = slotsForDate(time.date);
    const runs = await Promise.all(slots.map(async (slot) => ({
      slot, run: await readRunRecord(path.join(stateDirectory, claimFilename(time.date, slot))),
    })));
    const todayRuns = runs.map(({ run }) => run).filter(Boolean)
      .sort((a, b) => String(b.startedAt || "").localeCompare(String(a.startedAt || "")));
    let lastRun = todayRuns[0] || await readJson(path.join(stateDirectory, "latest.json"));
    const lastSlot = repairSlots.find((slot) => slot.id === lastRun?.slotId) || repairSlots[1];
    if (lastRun?.status === "claimed"
      && (lastRun.date !== time.date || verificationEnded(time, lastSlot))) {
      lastRun = {
        ...lastRun, status: "unverified",
        message: "The scheduled attempt ended without a confirmed result. Its claim is retained; this scheduled run will not be repeated.",
      };
    }
    const missedSlots = base.enabled ? runs.filter(({ slot, run }) => !run
      && (time.hour > slot.hour || (time.hour === slot.hour && time.minute > (slot.minute || 0))))
      .map(({ slot }) => slot.scheduledTime) : [];
    return { ...base, lastRun, todayRuns, missedSlots, missedToday: missedSlots.length > 0 };
  } catch {
    return { ...base, lastRun: null, statusError: "Morning repair status could not be read. No automatic retry is permitted for an existing scheduled-run claim." };
  }
}

function validCapture(value, sentAt, now) {
  const time = Date.parse(value || "");
  return Number.isFinite(time) && time >= Date.parse(sentAt) && time <= now.getTime() + 60_000;
}

export function summarizePmbRepairRefresh({ levels = {}, pricing = {} } = {}, { sentAt, now = new Date() } = {}) {
  const expected = Number.isSafeInteger(levels.expectedCount) && levels.expectedCount > 0 ? levels.expectedCount : 0;
  const items = Array.isArray(levels.items) ? levels.items : [];
  const liveItems = items.filter((item) => item.levelAvailable === true
    && item.fillLevelPercent !== null && item.fillLevelPercent !== ""
    && Number.isFinite(Number(item.fillLevelPercent))
    && Number(item.fillLevelPercent) >= 0 && Number(item.fillLevelPercent) <= 100);
  const captured = new Set(liveItems.map((item) => Number(item.tapNumber)).filter((tap) => Number.isSafeInteger(tap) && tap > 0)).size;
  const levelsFresh = !levels.error && levels.stale === false && validCapture(levels.updatedAt, sentAt, now);
  const levelsComplete = levelsFresh && !levels.partial && !levels.degraded
    && expected > 0 && captured === expected && items.length === expected && levels.capturedCount === expected;
  const pricingFresh = !pricing.error && !pricing.stale && !pricing.degraded
    && Array.isArray(pricing.items) && pricing.items.length > 0 && validCapture(pricing.updatedAt, sentAt, now);
  return {
    verified: levelsComplete && pricingFresh,
    levelsFresh, levelsComplete, pricingFresh,
    expectedCount: expected,
    capturedCount: levelsFresh ? captured : 0,
    missingTaps: [...new Set((Array.isArray(levels.unreachableTaps) ? levels.unreachableTaps : [])
      .map((item) => Number(item.tapNumber)).filter((tap) => Number.isSafeInteger(tap) && tap > 0))].sort((a, b) => a - b),
    levelBackupSaved: levels.sharedSnapshotSaved === true,
    pricingBackupSaved: pricing.pmbBackupSaved === true,
  };
}

export function createPmbMorningRepairRunner({
  env = process.env,
  now = () => new Date(),
  stateDirectory = defaultStateDirectory(),
  repair,
  refresh,
  wait = delay,
  recordActivity = (entry) => recordDashboardActivity(entry, { env }),
} = {}) {
  const running = new Set();
  return async function runScheduledRepair(resumeRecord = null) {
    const started = now();
    const slot = resumeRecord ? repairSlots.find((entry) => entry.id === resumeRecord.slotId) : currentRepairSlot(started);
    if (!isPmbMorningRepairEnabled(env) || !slot) return { skipped: true };
    const date = resumeRecord ? resumeRecord.date : getPmbRepairClock(started).date;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { skipped: true };
    const runKey = `${date}:${slot.id}`;
    if (running.has(runKey)) return { skipped: true };
    running.add(runKey);
    const claimPath = path.join(stateDirectory, claimFilename(date, slot));
    let claimed = false;
    let writeAttempted = false;
    let state = {
      date, startedAt: started.toISOString(), status: "claimed",
      attempt: 1,
      slotId: slot.id, scheduledTime: slot.scheduledTime,
      acknowledgeTapInterruption: true,
      authorization: `Owner-approved ${slot.id} ${slot.scheduledTime} Eastern scheduled repair, including tap interruption.`,
      message: `${slot.scheduledTime} Eastern repair claimed. Waiting to send the scheduled configuration update.`,
    };
    const save = async (update) => {
      state = { ...state, ...update, updatedAt: now().toISOString() };
      await atomicJson(claimPath, state);
      await atomicJson(`${claimPath}.backup.json`, state);
      await atomicJson(path.join(stateDirectory, "latest.json"), state);
    };
    const audit = async () => {
      await recordActivity({
        area: "Keg Levels", action: "scheduled PMB repair", role: "owner",
        summary: `${date} ${slot.scheduledTime} Eastern, owner-approved repair: ${state.message}`,
      }).then(async () => {
        await save({ activityBackupStatus: "saved" });
      }).catch(async () => {
        console.error("Scheduled PMB repair activity backup is unconfirmed; local run records are retained.");
        await save({ activityBackupStatus: "unconfirmed" }).catch(() => {});
      });
    };
    try {
      await mkdir(stateDirectory, { recursive: true, mode: 0o700 });
      if (resumeRecord) {
        const prior = await readRunRecord(claimPath);
        if (prior?.repairStatus !== "confirmed" || !Number.isFinite(Date.parse(prior.sentAt))
          || prior.status === "verified") return { skipped: true };
        state = prior;
        claimed = true;
        writeAttempted = true;
        if (typeof refresh !== "function") throw new Error("Missing verification adapter.");
        const remaining = Date.parse(state.sentAt) + 180_000 - now().getTime();
        if (remaining > 0) await wait(remaining);
      } else {
      let file;
      try {
        file = await open(claimPath, "wx", 0o600);
        claimed = true;
        await file.writeFile(`${JSON.stringify(state)}\n`);
        await file.sync();
      } catch (error) {
        if (error.code !== "EEXIST") throw error;
        let prior;
        try { prior = await readJson(claimPath); }
        catch { return { skipped: true, alreadyClaimed: true, statusError: "Existing repair claim could not be read; no retry is permitted." }; }
        if (prior?.status !== "retryable" || !Number.isInteger(prior.attempt) || prior.attempt >= 3
          || !Number.isFinite(Date.parse(prior.updatedAt)) || started.getTime() - Date.parse(prior.updatedAt) < 15_000) {
          return { skipped: true, alreadyClaimed: true };
        }
        state.attempt = prior.attempt + 1;
        // A separate exclusive claim prevents concurrent processes retrying the same attempt.
        try { file = await open(`${claimPath}.attempt-${state.attempt}`, "wx", 0o600); }
        catch (retryError) {
          if (retryError.code === "EEXIST") return { skipped: true, alreadyClaimed: true };
          throw retryError;
        }
        claimed = true;
        await file.writeFile(JSON.stringify(state));
        await file.sync();
      } finally {
        await file?.close();
      }
      await save({ status: "claimed" });
      if (typeof repair !== "function" || typeof refresh !== "function") throw new Error("Missing scheduled repair adapters.");
      const beforeWrite = async () => {
        const checkedAt = now();
        if (!isPmbMorningRepairEnabled(env) || currentRepairSlot(checkedAt)?.id !== slot.id || getPmbRepairClock(checkedAt).date !== date) {
          const error = new Error(`The ${slot.scheduledTime} Eastern repair window has closed.`);
          error.code = "PMB_REPAIR_WINDOW_CLOSED";
          throw error;
        }
        writeAttempted = true;
      };
      // The owner has pre-authorized the interruption during this scheduled
      // pre-opening window. Manual repairs still require guest-clear confirmation.
      // The shared PMB adapter rechecks the window before each config POST.
      let preparation;
      try { preparation = await readJson(path.join(stateDirectory, preparationFilename(date, slot))); }
      catch { preparation = { status: "needs-attention", code: "PREPARATION_STATUS_UNREADABLE" }; }
      await save({ productPreparation: preparation || { status: "not-run" } });
      await repair({ beforeWrite });
      await save({ status: "repair-sent", repairStatus: "confirmed", sentAt: now().toISOString(), message: "Scheduled repair sent. Waiting three minutes for the tap walls to reconnect." });
      await audit();
      await wait(180_000);
      }
      for (let attempt = (Number(state.verificationAttempts) || 0) + 1; ; attempt += 1) {
        if (!isPmbMorningRepairEnabled(env)) {
          await save({ status: "unverified", message: "Repair was sent. Read-only verification is paused while the scheduler is disabled and will resume when enabled." });
          break;
        }
        let result;
        try { result = await refresh(); } catch { result = {}; }
        const verification = summarizePmbRepairRefresh(result, { sentAt: state.sentAt, now: now() });
        const coverage = verification.expectedCount ? `${verification.capturedCount}/${verification.expectedCount} taps verified` : "live tap coverage unavailable";
        await save({
          status: verification.verified ? "verified" : "verifying",
          completedAt: verification.verified ? now().toISOString() : null,
          verification, verificationAttempts: attempt,
          levelsStatus: verification.levelsComplete ? "verified" : "unverified",
          pricingStatus: verification.pricingFresh ? "verified" : "unverified",
          backupStatus: verification.levelBackupSaved && verification.pricingBackupSaved ? "saved" : "unconfirmed",
          message: verification.verified
            ? `Repair complete: ${coverage}; current tap pricing refreshed.`
            : `Repair sent: ${coverage}; ${verification.pricingFresh ? "pricing refreshed" : "current pricing unverified"}. Retrying read-only checks until verified; this scheduled repair will not be repeated.`,
        });
        if (verification.verified) break;
        await wait(60_000);
      }
      await audit();
      return state;
    } catch (error) {
      if (!claimed) return { status: "failed", message: "Morning repair was not sent because its scheduled-run claim could not be saved." };
      const canRetry = !writeAttempted && state.attempt < 3
        && currentRepairSlot(now())?.id === slot.id && getPmbRepairClock(now()).date === date;
      const status = writeAttempted ? "uncertain" : canRetry ? "retryable" : error.code === "PMB_REPAIR_WINDOW_CLOSED" ? "missed-window" : "failed";
      const message = writeAttempted
        ? "The scheduled repair or its verification could not be confirmed. This scheduled run will not be repeated; other scheduled runs are unchanged."
        : canRetry ? "No configuration write was attempted. A bounded retry is allowed only within this scheduled minute."
          : "Morning repair was not sent. Its safe window ended or a required connection was unavailable; this run will not catch up later. Other scheduled runs are unchanged.";
      await save({ status, message }).catch(() => {});
      await audit();
      return { ...state, status, message };
    } finally {
      running.delete(runKey);
    }
  };
}

export function startPmbMorningRepairScheduler(options = {}) {
  if (!isPmbMorningRepairEnabled(options.env || process.env)) return null;
  if (globalThis[schedulerKey]) return globalThis[schedulerKey];
  const run = createPmbMorningRepairRunner(options);
  const prepare = createPreparationRunner(options);
  const preparationTimer = setInterval(() => { void prepare(); }, 15_000);
  preparationTimer.unref();
  void prepare();
  const report = (result) => {
    if (result && !result.skipped) console.info("PMB morning repair:", result.status, result.message);
  };
  let scanning = false;
  const recover = async () => {
    if (scanning || !isPmbMorningRepairEnabled(options.env || process.env)) return;
    scanning = true;
    try {
      const directory = options.stateDirectory || defaultStateDirectory();
      const files = await readdir(directory).catch((error) => {
        if (error.code === "ENOENT") return [];
        throw error;
      });
      for (const file of files) {
        if (!/^\d{4}-\d{2}-\d{2}(?:-nightly|-monday-reset)?\.json$/.test(file)) continue;
        const record = await readRunRecord(path.join(directory, file));
        if (record?.repairStatus === "confirmed" && record.status !== "verified") {
          void run(record).then(report).catch(() => console.error("PMB repair verification recovery failed."));
        }
      }
    } finally { scanning = false; }
  };
  const tick = () => {
    void run().then(report).catch(() => console.error("PMB morning repair scheduler could not complete its check."));
    void recover().catch(() => console.error("PMB repair verification records could not be read."));
  };
  const timer = setInterval(tick, 15_000);
  timer.unref();
  globalThis[schedulerKey] = { stop() { clearInterval(timer); clearInterval(preparationTimer); delete globalThis[schedulerKey]; } };
  // Restart recovery resumes only read-only verification, never a missed repair.
  tick();
  return globalThis[schedulerKey];
}
