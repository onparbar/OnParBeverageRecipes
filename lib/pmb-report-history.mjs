...
export function parseReportHistoryRequest(params) {
  if ([...params.keys()].some(key => !['view', 'plus', 'name'].includes(key))) throw Object.assign(new Error('Unsupported report parameter.'), { status: 422 });
  const view = params.get('view') || 'catalog';
  if (!['catalog', 'menu', 'home', 'reporting', 'products'].includes(view)) throw Object.assign(new Error('Choose catalog, menu, home, or products.'), { status: 422 });
...
export async function readPmbReportHistory(input, { config = getConfig(), readPage = resilientRead, readProduct = readProductForm } = {}) {
  ...
  if (input.view === 'reporting') delete described.tables;
  return described;
}
