...
test("combines repeated beer and cocktail products across tap walls", () => {
  ...
  assert.deepEqual(plan.prep.cocktails.map((item) => ({ name: item.name, quantity: item.quantity, taps: item.tapNumbers })), [
    { name: "House Margarita", quantity: 2, taps: [50, 94] },
  ]);
...
test("keeps liquor, mixer, and supply orders together with case quantities", () => {
  const plan = buildWeeklyActionPlan({
    ...
    recommendations: [
      { actionType: "review", isLiquorTap: true, orderQty: 0, deferredQty: 1, deferredReview: true, orderProductName: "Hennessy 3", tapNumber: 1, wall: "Patio", currentStockOunces: 80, avgWeeklyOunces: 120 },
    ],
  });
  ...
  assert.equal(plan.review.deferredLiquorRefills[0].name, "Hennessy");
  assert.equal(plan.summary.orderLineCount, 3);
  assert.equal(plan.summary.heldLineCount, 1);
  assert.equal(plan.summary.estimatedInventoryCost, 122);
});

test("holds PMB liquor-tap refill recommendations while refill netting is deferred", () => {
  const plan = buildWeeklyActionPlan({
    inventoryItems: [
      { id: "hennessy-cabinet", name: "Hennessy", group: "Liquor Cabinet", orderUnits: 2, unitCost: 35, estimatedCost: 70 },
    ],
    recommendations: [
      { actionType: "review", isLiquorTap: true, orderQty: 0, deferredQty: 1, deferredReview: true, orderProductName: "Hennessy 3", tapNumber: 1, wall: "Patio" },
    ],
  });

  assert.deepEqual(plan.orders.liquor.map((item) => item.name), ["Hennessy"]);
  assert.equal("liquorRefills" in plan.orders, false);
  assert.deepEqual(
    plan.review.deferredLiquorRefills.map((item) => ({ name: item.name, quantity: item.quantity })),
    [{ name: "Hennessy", quantity: 1 }],
  );
  assert.equal(plan.summary.orderLineCount, 1);
  assert.equal(plan.summary.estimatedKnownPurchaseCost, 70);
...
test("shows inventory ordering holds without adding them to active orders", () => {
  ...
  assert.deepEqual(plan.review.excludedInventory.map((item) => item.name), ["Bombay Sapphire"]);
...
test("readiness blocks incomplete inputs, marks changed inputs stale, and exposes review warnings", () => {
  const base = {
    parInitialized: true,
  ...
  assert.equal(evaluateWeeklyPlanReadiness({ ...base, recommendationSourceCurrent: false }).status, "stale");
