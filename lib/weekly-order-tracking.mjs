...
const LEGACY_ORDER_CATALOG = Object.freeze([{
  ...
  vendor: "Heidelberg",
  vendorSku: "13452",
  vendorProductName: "Athletic Brewing Upside Dawn Non-Alcoholic Golden",
  lineType: "Supply",
...
const THURSDAY = 4;
const THURSDAY_DELIVERY_MINUTE = 9 * 60;
...
const EASTERN_WEEKDAYS = Object.freeze({
  Sun: 0,
  Mon: 1,
  Tue: 2,
  Wed: 3,
...
function easternClockParts(value) {
  const date = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(date.getTime())) return null;
  const parts = Object.fromEntries(
    EASTERN_CLOCK_FORMATTER.formatToParts(date)
      .filter((part) => part.type !== "literal")
      .map((part) => [part.type, part.value]),
  );
  return {
    date,
    weekday: EASTERN_WEEKDAYS[parts.weekday],
    minuteOfDay: (Number(parts.hour) * 60) + Number(parts.minute),
  };
}

function nextThursdayDeliveryTime(value) {
  const clock = easternClockParts(value);
  if (!clock || !Number.isInteger(clock.weekday)) return null;
  let daysUntilThursday = (THURSDAY - clock.weekday + 7) % 7;
  if (daysUntilThursday === 0 && clock.minuteOfDay >= THURSDAY_DELIVERY_MINUTE) {
    daysUntilThursday = 7;
  }
  const minutesUntilDelivery = (daysUntilThursday * 24 * 60)
    + THURSDAY_DELIVERY_MINUTE
    - clock.minuteOfDay;
  return clock.date.getTime() + (minutesUntilDelivery * MINUTE_MS);
}

function isBeforeThisWeeksThursdayDelivery(value) {
  const clock = easternClockParts(value);
  if (!clock || !Number.isInteger(clock.weekday)) return false;
  return clock.weekday >= 1
    && (clock.weekday < THURSDAY
      || (clock.weekday === THURSDAY && clock.minuteOfDay < THURSDAY_DELIVERY_MINUTE));
}

function receivedInventoryUnits(item, receivedQuantity) {
  const quantity = nonNegativeNumber(receivedQuantity);
  const isCaseQuantity = Boolean(item.casePackaged) || nonNegativeNumber(item.requestedCases) > 0;
  return isCaseQuantity
    ? quantity * Math.max(1, nonNegativeNumber(item.packSize))
    : quantity;
}

function classifyDeliveryShortage(item = {}, context = {}) {
  ...
  const receivedQuantity = nonNegativeNumber(context.receivedQuantity);
  const currentStockKegs = nonNegativeNumber(item.currentStockKegs);
  const avgWeeklyKegs = nonNegativeNumber(item.avgWeeklyKegs);
  const isBeerTap = lineType === "beer keg"
    || (tapNumbers.length > 0 && (currentStockKegs > 0 || avgWeeklyKegs > 0));

  if (isBeerTap) {
    const now = context.now instanceof Date ? context.now : new Date(context.now || Date.now());
    const deliveryTime = nextThursdayDeliveryTime(now);
    const generatedTime = new Date(context.generatedAt || now).getTime();
    const baselineTime = Number.isFinite(generatedTime) ? generatedTime : now.getTime();
    const daysToDelivery = deliveryTime === null
      ? 0
      : Math.max(0, deliveryTime - baselineTime) / DAY_MS;
    const projectedUse = avgWeeklyKegs * (daysToDelivery / 7);
    const stockThroughDelivery = currentStockKegs + receivedQuantity;
    const willRunOut = avgWeeklyKegs > 0
      ? stockThroughDelivery <= projectedUse
      : stockThroughDelivery <= 0;

    return willRunOut
      ? {
        priority: "critical",
        reason: "Connected and on-hand stock is projected to run out before Thursday's replacement delivery.",
      }
      : {
        priority: "wait",
        reason: "Connected and on-hand stock is projected to last until Thursday's replacement delivery.",
      };
  }

  if (lineType === "liquor tap bottle") {
    return {
      priority: "critical",
      reason: "Needed for this week's liquor tap plan.",
    };
  }

  const prepRequired = nonNegativeNumber(item.cocktailPrepRequiredBottles);
  const prepShortage = nonNegativeNumber(item.cocktailPrepShortageUnits);
  const hasCocktailPrepNeed = prepRequired > 0 || prepShortage > 0 || isCurrentWeekPrep;

  if (hasCocktailPrepNeed && !isVendorMinimumTopUp) {
    const explicitlyNeededEarly = Boolean(
      item.neededBeforeThursday
      || item.neededBeforeNextDelivery
      || item.requiredBeforeThursday
      || item.requiredBeforeNextDelivery,
    );
    const neededBeforeDelivery = explicitlyNeededEarly
      || !isBeforeThisWeeksThursdayDelivery(context.now || new Date());
    const onHand = nonNegativeNumber(item.onHand);
    const available = onHand + receivedInventoryUnits(item, receivedQuantity);
    const required = prepRequired > 0 ? prepRequired : onHand + prepShortage;
    const cannotCoverPrep = available < required;

    if (neededBeforeDelivery && cannotCoverPrep) {
      return {
        priority: "critical",
        reason: "Available inventory cannot cover cocktail prep needed before the next Thursday delivery.",
      };
    }

    return {
      priority: "wait",
      reason: neededBeforeDelivery
        ? "Available inventory covers cocktail prep through the next Thursday delivery."
        : "This cocktail ingredient is not needed before Thursday's delivery.",
    };
  }
  ...
  const vendors = orderModel.drafts.map((group) => {
    ...
    const items = group.lines.map((item) => {
      ...
      const status = savedStatus === "pending"
        ...
        kegAllocations: Array.isArray(receipt.kegAllocations) ? receipt.kegAllocations : [],
...
export function applyWeeklyOrderTrackingUpdate(
  ...
  if (["set-order-adjustment", "set-order-adjustments", "remove-order-adjustment"].includes(payload.action)) {
  ...
  } else if (payload.action === "reopen-draft") {
    if (role !== "owner") throw new WeeklyOrderTrackingError("OWNER_ORDER_DRAFT_REQUIRED", "Only an owner can reopen an order draft.", 403);
    if (current.vendors.some((vendor) => vendor.vendor === clean(payload.vendor) && vendor.ordered)) {
      throw new WeeklyOrderTrackingError("ORDER_ALREADY_PLACED", "This order is marked as placed. Resolve its placement status before editing; reopening cannot cancel a vendor order.", 409);
    }
    ...
    if (!target?.approvedAt) throw new WeeklyOrderTrackingError("ORDER_DRAFT_NOT_APPROVED", "That approved draft is no longer available to reopen.", 409);
    const actor = clean(payload.adjustedBy).slice(0, MAX_NAME_LENGTH);
    if (!actor) throw new WeeklyOrderTrackingError("ORDER_ADJUSTMENT_ACTOR_REQUIRED", "Enter the manager reopening this draft.");
    const next = {
      ...target,
      ...
      reopenedBy: actor,
      reopenHistory: [...(Array.isArray(target.reopenHistory) ? target.reopenHistory : []), {
        approvedAt: target.approvedAt,
        approvedBy: target.approvedBy,
        copiedAt: target.copiedAt || "",
    ...
    delete next.copiedAt;
    delete next.openedAt;
    delete next.statusActor;
    ...
    if (payload.expectedDraftId && clean(payload.expectedDraftId) !== draft.id) {
      throw new WeeklyOrderTrackingError("ORDER_DRAFT_CHANGED", "This order changed. Reload its items before opening it.", 409);
    }
    ...
    if (payload.ordered !== true) {
      ...
      if (payload.expectedDraftId) {
        const snapshot = getCurrentWeeklyPlanSnapshot(recommendations, now());
        const model = buildUnifiedVendorOrderModel(snapshot?.plan, {
          snapshot, orderPolicy, manualAdjustments: current.adjustments,
          manualCatalog: current.adjustmentCatalog, now: now(),
        });
        const draft = model.drafts.find((entry) => entry.vendor === target.vendor);
        if (!draft || draft.id !== clean(payload.expectedDraftId)) {
          throw new WeeklyOrderTrackingError("ORDER_DRAFT_CHANGED", "This order changed. Reload before marking it placed.", 409);
        }
      }
    ...
    } else if (["set-receipts", "set-selected-receipts"].includes(payload.action)) {
    const completeVendorReview = payload.action === "set-receipts";
    ...
    if (payload.confirmed !== true) {
      throw new WeeklyOrderTrackingError(
        ...
        completeVendorReview
          ...
          : "Review and confirm the selected delivery items before saving.",
      );
    }
    ...
    if (!entries.length || entries.length > 100 || (completeVendorReview && entries.length !== targetVendor.items.length)) {
      throw new WeeklyOrderTrackingError(
        completeVendorReview ? "RECEIPT_BATCH_INCOMPLETE" : "RECEIPT_SELECTION_REQUIRED",
        completeVendorReview
          ? "The reviewed delivery must include every item for this vendor."
          : "Choose between 1 and 100 delivery items to save.",
      );
    }
    ...
    if (completeVendorReview && seenItemIds.size !== targetVendor.items.length) {
      throw new WeeklyOrderTrackingError(
