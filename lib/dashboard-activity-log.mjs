...
export async function recordDashboardActivity({ area, action, role = "owner", revision = 0, summary, dedupe = false }, { env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  if (dedupe) {
    const lookupUrl = getTableUrl(config);
    lookupUrl.searchParams.set("select", "id");
    lookupUrl.searchParams.set("area", `eq.${entry.area}`);
    lookupUrl.searchParams.set("action", `eq.${entry.action}`);
    lookupUrl.searchParams.set("role", `eq.${entry.role}`);
    lookupUrl.searchParams.set("summary", `eq.${entry.summary}`);
    ...
    if (!lookup.ok || !Array.isArray(rows)) throw new Error("Shared activity storage is unavailable.");
