...
export function buildRawRecommendation(tap, level, weeklyEntries, state, settings, sharedWeeklyUsageItem = null) {
  ...
  if (isCocktailTap(tap)) {
    const cocktailNeed = buildOperationalRecommendation({
      kind: "cocktail",
      wall: tap.wall,
      averageUsage: avgWeeklyKegs,
      reserve: COCKTAIL_RESERVE_KEGS,
      position: buildInventoryPosition({ connected: currentStockKegs }),
    });
  ...
  const beerNeed = buildOperationalRecommendation({
    kind: "beer",
    wall: tap.wall,
    averageUsage: avgWeeklyKegs,
    reserve: beerReserveKegs,
