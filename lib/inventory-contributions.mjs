...
export function buildRecipeInventoryContributions(recipe, catalog, target = {}) {
  ...
  inventoryIngredients.forEach((ingredient) => {
    ...
    // Excluded juices stay in the batch yield so counted ingredients scale correctly.
    if (/^(?:strawberry lemonade|cranberry(?: juice)?)$/i.test(clean(ingredient.name))) return;
