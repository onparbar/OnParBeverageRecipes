...
export async function readInventoryState() {
  try {
    ...
    if (error?.code === "ENOENT") return createEmptyInventoryState();
    throw error;
  }
}
...
export function saveInventorySnapshot(items, role = "owner", now = new Date(), summary = null, kegPlanSnapshot = null) {
  return queueUpdate((state) => (
    applyInventoryStateAction(state, "save-snapshot", { items, summary, kegPlanSnapshot }, role, now)
  ));
}
