...
export function reorderInventoryItems(itemOrder, role = "owner") {
  return queueUpdate((state) => {
    state.current.itemOrder = sanitizeItemOrder(itemOrder);
    return markCurrentUpdated(state, role);
  });
}
...
export function deleteCustomInventoryItemState(id, role = "owner") {
  return queueUpdate((state) => {
    ...
    state.current.itemOrder = state.current.itemOrder.filter((itemId) => itemId !== normalizedId);
...
export function saveInventorySnapshot(items, role = "owner", now = new Date(), summary = null) {
  return queueUpdate((state) => {
    ...
    if (!snapshot.summary
      ...
      throw new Error("Complete PMB tap coverage is required for a Monday inventory snapshot.");
    }
    state.snapshots = [
      snapshot,
